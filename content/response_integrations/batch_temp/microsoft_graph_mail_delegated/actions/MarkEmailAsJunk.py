...
def _mark_emails_as_junk(
    manager: api_manager.ApiManager,
    emails_to_mark_as_junk: MutableSequence[MicrosoftGraphEmail],
) -> None:
    for email in emails_to_mark_as_junk:
        manager.mark_email_as_junk(email)


class MarkEmailAsJunk(Action[api_manager.ApiManager]):

    def __init__(self) -> None:
        super().__init__(constants.MARK_EMAIL_AS_JUNK_SCRIPT_NAME)
        self.output_message = "Successfully marked the email as junk.\n\n"
        self.error_output_message = (
            f'Error executing action "{constants.MARK_EMAIL_AS_JUNK_SCRIPT_NAME}".'
        )
    ...
    def _perform_action(self, _) -> None:
        ...
        emails_to_mark_as_junk = self._get_validated_emails(
            folders=mailbox_result.valid_mailboxes
        )
        _mark_emails_as_junk(
            manager=self.api_client,
            emails_to_mark_as_junk=emails_to_mark_as_junk,
        )
