...
BLOCKFILE_SCRIPT_NAME = f"{INTEGRATION_NAME} - BlockFile"
...
def main():
    ...
    try:
        ...
        if use_v2_api:
            netskope_manager.block_file(file_id)
        else:
            netskope_manager.block_file(file_id, quarantine_profile_id)
        result_value = True
        status = EXECUTION_STATE_COMPLETED
        output_message = f"Successfully blocked file {file_id}"
        ...
        output_message = f'Error executing action "BlockFile". Reason: {e}'
