...
from .exceptions import ServiceNowConnectorException
...
def load_processed_ids(connector_scope: SiemplifyConnectorExecution) -> list[str]:
    """Loads processed IDs.
    Args:
        connector_scope(SiemplifyConnectorExecution): Connector scope.
    Returns:
        list[str]: List of processed IDs.
    """
    try:
        return list(read_ids(connector_scope))
    except Exception as e:
        raise ServiceNowConnectorException(
            f"Failed to read processed incidents. Error: {e}"
        ) from e


@dataclasses.dataclass(slots=True)
class FetchResult:
    """Dataclass for storing fetch results."""
    incidents: list[Incident]
    latest_timestamp: str | None
    ids_to_persist: list[str]


def fetch_and_filter_incidents(
    ...
    """Fetches incidents from API, filters, and limits them.
    Args:
        sn_manager(ServiceNowManager): ServiceNowManager instance.
        filter_params(SingleJson): Filter parameters.
        existing_ids(list[str]): List of existing IDs.
        field_name(str): Field name.
        max_incidents_per_cycle(int): Max incidents per cycle.
    Returns:
        FetchResult: A dataclass containing:
            - incidents(list[Incident]): List of filtered incidents.
            - latest_timestamp(str | None): The effective latest timestamp.
            - ids_to_persist(list[str]): List of IDs with that timestamp to persist.
    """
    incidents: list[Incident] = sn_manager.get_incidents_by_filter(
        **filter_params, limit=max_incidents_per_cycle
    )
    existing_ids_set: set[str] = set(existing_ids)
    filtered_incidents: list[Incident] = [
        inc for inc in incidents if inc.sys_id not in existing_ids_set
    ]

    latest_timestamp: str | None = None
    if incidents:
        latest_timestamp = incidents[-1].raw_data.get(field_name)

    result_incidents: list[Incident] = filtered_incidents[:max_incidents_per_cycle]

    effective_latest_timestamp: str | None = latest_timestamp
    if result_incidents:
        effective_latest_timestamp = result_incidents[-1].raw_data.get(field_name)

    ids_to_persist = []
    for inc in incidents:
        if inc.raw_data.get(field_name) == effective_latest_timestamp:
            if inc.sys_id in existing_ids_set:
                ids_to_persist.append(inc.sys_id)
            elif inc in result_incidents:
                ids_to_persist.append(inc.sys_id)

    return FetchResult(
        incidents=result_incidents,
        latest_timestamp=effective_latest_timestamp,
