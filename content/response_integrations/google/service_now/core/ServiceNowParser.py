...
class ServiceNowParser:
    ...
    def build_cmdb_record_details_object(raw_data):
        ...
        return CMDB_Record_Detail(
            ...
            inbound_relations=[
                ...
                for inbound_relation in raw_data.get("result", {}).get(
                    "inbound_relations", {}
                )
            ],
            outbound_relations=[
                ServiceNowParser.build_cmdb_record_outbound_object(outbound_relation)
                for outbound_relation in raw_data.get("result", {}).get(
                    "outbound_relations", {}
                )
            ],
        )
    ...
    def build_attachments_object(self, raw_data, download_folder_path):
        return [
            self.get_attachment(
                raw_data=attachment_data, download_folder_path=download_folder_path
            )
    ...
    def build_user_related_records(self, raw_data):
        return [
            self.get_user_related_record(raw_data=record)
    ...
    def build_results(self, raw_json, method):
        return [
            getattr(self, method)(item_json) for item_json in raw_json.get("result", [])
        ]
