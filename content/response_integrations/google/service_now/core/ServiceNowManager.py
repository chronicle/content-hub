...
from .exceptions import (
    ...
    ServiceNowNoRecordFoundException,
...
class ServiceNowManager:
    ...
    def remove_version_from_apiroot(self, api_root, version_to_remove):
        ...
        """
        api_root = (
            urllib.parse.urlparse(api_root)
            .geturl()
            .replace(f"/{version_to_remove}", "")
        )
        return api_root
    ...
    def upload_attachment(self, table_name, record_sys_id, file_path):
        ...
        """
        upload_attachment_url = self._get_full_url(
            "attachment_upload", general_api=False
        )
    ...
    def get_user_related_data(
        self, user_id, max_records, max_days_backward, table_name=None
    ):
        """
        Find user_sys_id base on on user_name
        ...
        search_params = f"opened_by={user_id}^sys_created_onRELATIVEGT@dayofweek@ago@{max_days_backward}"
    ...
    def get_incidents(
        ...
        if numbers:
            number_query = self.__get_param_query_fragment(
                "number", numbers, join_char="^OR"
            )
    ...
    def get_ticket_comments(
        ...
        sys_param_query = self.__get_param_query_fragment(
            "element", param_keys, join_char="^OR"
        )
    ...
    def check_incidents(self, incident_numbers: list[str]) -> list:
        ...
        """
        incident_numbers_to_query = [
            f"number={inc_number}" for inc_number in incident_numbers
        ]
        search_params = self.__build_system_query(
            incident_numbers_to_query, join_char="^OR"
        )
        response = self.session.get(
    ...
    def create_object(self, json_data=None, table_name=None):
        ...
        if json_data:
            ...
            if "assignment_group" in json_data_keys:
                json_data["assignment_group"] = self.get_group_id(
                    json_data["assignment_group"]
                )
        ...
        for key, value in json_data.items():
            if key not in service_now_object_json or (
                key in service_now_object_json
                and service_now_object_json.get(key) != value
            ):
    ...
    def get_record_details(self, ticket_sys_id, fields, table_name=None):
        ...
        table_name = self.get_table_name(table_name)
        url = self._get_full_url(
            "get_record_details", table=table_name, sys_id=ticket_sys_id
        )
    ...
    def update_incident(
        ...
        for key in list(update_payload.keys()):
            ...
            if key in ["assigned_to", "assignment_group", "cmdb_ci"]:
                ...
                continue
            if (
                key in updated_ticket_json
                and update_payload[key] != updated_ticket_json[key]
            ):
                raise ServiceNowException(
    ...
    def close_incident(
        self, ticket_number, close_reason, close_notes=None, close_code=None
    ):
        """
    ...
    def get_incidents_by_filter(
        ...
        Args:
            domain (str | None): Filter incidents within this domain name.
            close_status (str | None): Filter incidents with this close status
                number (e.g., '7' for closed).
            updated_time (str | None): Filter incidents updated after this UTC
                timestamp (e.g., '2018-06-27 11:46:09').
            table_name (str | None): The name of the table to query.
            custom_queries (list[str] | None): A list of custom query strings
                to apply.
            sys_id (str | None): Filter incidents assigned to this sys_id.
            limit (int | None): The maximum number of incidents to retrieve.

        Returns:
            list[datamodels.Incident]: A list of Incident objects matching the
                filter criteria.
        """
        ...
        if domain:
            ...
            query_parts.append(f"sys_domain={domain_id}")

        if close_status:
            query_parts.append(f"active=false^state={close_status}")
        else:
            query_parts.append("active=true")

        if updated_time:
            query_parts.append(f"sys_updated_on>={updated_time}")

        if custom_queries:
            query_parts.extend(custom_queries)

        if sys_id:
            query_parts.append(f"assignment_group={sys_id}")

        search_params: str = "^".join(query_parts)
        if updated_time:
            search_params += "^ORDERBYsys_updated_on^ORDERBYsys_id"
        url: str = self._get_full_url(
            "get_incidents", table=self.get_table_name(table_name)
        )
        params: dict[str, str | int] = {"sysparm_query": search_params}
        return self._fetch_incidents_with_pagination(url, params, limit)

    def _fetch_incidents_with_pagination(
        self,
        url: str,
        params: dict[str, str | int],
        limit: int | None = None,
    ) -> list[datamodels.Incident]:
        """Fetch incidents from ServiceNow in chunks of page_size.
        Args:
            url (str): The API URL.
            params (dict): The initial parameters (containing sysparm_query).
            limit (int | None): Total limit of incidents to fetch.

        Returns:
            list[datamodels.Incident]: A list of Incident objects.
        """
        all_incidents: list[datamodels.Incident] = []
        total_limit: int = limit if limit else 1000
        page_size: int = 100
        offset: int = 0

        while len(all_incidents) < total_limit:
            current_page_limit: int = min(page_size, total_limit - len(all_incidents))
            params["sysparm_limit"] = current_page_limit
            ...
            try:
                ...
                results: list = json_data.get("result", [])
            except ServiceNowNoRecordFoundException:
                break

            if not results:
                break

            all_incidents.extend(self.parser.build_incidents(json_data))

            if self.siemplify_logger:
                self.siemplify_logger.info(
                    f"Fetched {len(results)} incidents in this page. "
                    f"Total so far: {len(all_incidents)}"
                )

            if len(results) < current_page_limit:
                break
    ...
    def get_incident_comments(self, ticket_number, table_name=None):
        ...
        ticket_id = self.get_ticket_id(
            ticket_number, table_name=self.get_table_name(table_name)
        )
    ...
    def get_attachments_info(
        ...
        return self.parser.build_attachments_object(
            response.json(), download_folder_path
        )
    ...
    def delete_attachment(self, sys_id: str):
        ...
        response = self.session.delete(
            self._get_full_url("attachment_details", sys_id=sys_id)
        )
    ...
    def get_attachment_content(self, download_link):
        ...
        try:
            ...
            raise ServiceNowException(
                f"An error occurred. ERROR: {e}. {request.content}"
            )
    ...
    def check_and_raise_not_found_exceptions(cls, exception):
        ...
        """
        error_message = (
            cls.get_api_error_message(exception) or exception.response.content.decode()
        )

        if "Invalid table" in error_message:
            raise ServiceNowTableNotFoundException(error_message)
        if "No Record found" in error_message:
            raise ServiceNowNoRecordFoundException(error_message)
    ...
    def validate_response(
        cls, response, error_msg="An error occurred", force_json_result=True
    ):
        """
        Validate response
    ...
    def get_incident_attachments_by_datetime(self, sys_id: str, created_on: str):
        ...
        response = self.session.get(
            self._get_full_url("download-attachment"), params=params
        )
        self.validate_response(response)
    ...
    def add_attachment_to_incident(
        ...
        self.session.headers.update(
            {"Content-Type": content_type or "multipart/form-data"}
        )
    ...
    def get_incidents_affected_cis(self, incident_sys_ids: list[str]):
        ...
        """
        params = {
            "sysparm_query": f"taskIN{convert_list_to_comma_string(incident_sys_ids)}"
        }

        response = self.session.get(
            self._get_full_url("get_incidents_affected_cis"), params=params
        )

        self.validate_response(response)
        return self.parser.build_results(response.json(), "build_affected_ci")

    def get_affected_cis_details(
        self, sys_ids: list[str], updated_on: str | None = None
    ):
        ...
        response = self.session.get(
            self._get_full_url("get_affected_cis_details"), params=params
        )
