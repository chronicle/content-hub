...
def close_incidents_in_servicenow(siemplify, sn_manager, last_execution_time):
    ...
    siemplify.LOGGER.info(
        "--- Start synchronize closure from Google SecOps to ServiceNow ---"
    )

    last_execution_time_sn = sn_manager.convert_datetime_to_sn_format(
        last_execution_time
    )
    ...
    siemplify.LOGGER.info(
        f"Found {len(closed_cases)} closed cases since {last_execution_time_sn}"
    )
    ...
    try:
        ...
        for incident in incidents_to_close:
            try:
                ...
                siemplify.LOGGER.info(
                    f"Incident {incident.number} closed in Service now"
                )
            except Exception as e:
                siemplify.LOGGER.error(
                    f"Failed to close incident {incident.number}. Reason: {e}"
                )
    ...
    siemplify.LOGGER.info(
        "--- Finish synchronize closure from Google SecOps to ServiceNow ---"
    )
...
def close_cases_in_siemplify(siemplify, sn_manager):
    ...
    """
    siemplify.LOGGER.info(
        "--- Start synchronize closure from ServiceNow to Google SecOps ---"
    )
    ...
    open_cases = [
        siemplify._get_case_by_id(case_id) for case_id in fetched_open_cases_ids
    ]
            ...
            for case_id, alert_ids in get_case_and_alerts_ids(case).items():
                for alert_id in alert_ids:
                    try:
                        ...
                        siemplify.LOGGER.info(
                            f"Alert for incident {incident.number} was closed"
                        )
                    except Exception as e:
                        siemplify.LOGGER.exception(e)
                        siemplify.LOGGER.error(
    ...
    siemplify.LOGGER.info(
        "--- Finish synchronize closure from ServiceNow to Google SecOps ---"
    )
