...
class SyncIncidentsJob(Job):
    ...
    def get_cache_data(self):
        ...
        for incident_number, case_ids in self._cached_incident_numbers_mapping.items():
            for case_id in case_ids:
                self._cached_incident_numbers_reverse_mapping.setdefault(
                    case_id, []
                ).append(incident_number)
        ...
        self.logger.info(
            f"Fetched {len(self._cached_incidents_data)} cached incidents data from "
            f"Job context"
        )
        ...
        self.logger.info(
            f"Fetched {len(self._cached_affected_cis)} cached affected CIs "
            f"from Job context"
        )
        ...
        if additional_incident_numbers:
            for incident_number in additional_incident_numbers:
                self._cached_incident_numbers_mapping[incident_number] = list(
                    set(self._cached_incident_numbers_mapping.get(incident_number, []))
                    - {case_id}
                )
    ...
    def set_incident_numbers_mapping(
        self, case_id: int, incident_numbers: [str]
    ) -> None:
    ...
    def get_incidents(self, incident_numbers, datetime=None):
        ...
        incidents = []
        sn_format_timestamp = (
            self.api_client.convert_datetime_to_sn_format(datetime)
            if datetime else None
        )
            ...
            if isinstance(value, dict) and value.get("link"):
                self._related_objects_mapping[value.get("link")] = {
                    "incident_numbers": list(
                        set(
                            self._cached_related_objects_mapping.get(
                                value.get("link"), {}
                            ).get("incident_numbers", [])
                            + self._related_objects_mapping.get(
                                value.get("link"), {}
                            ).get("incident_numbers", [])
    ...
    def get_incident_affected_cis_mapping(self):
        ...
        for split_incident_sys_ids in [
            ...
            for i in range(
                0, len(incident_sys_ids_mapping.keys()), AFFECTED_CIS_REQUEST_LIMIT
            )
        ]:
            try:
                affected_cis.extend(
            ...
            **self._incident_numbers_mapping,
            ...
            for scope_id in scope_ids:
                if self._incidents_changes.get(incident_number):
                    for key, change in self._incidents_changes.get(
                        incident_number
                    ).items():
                        self.logger.info(
                            f"Adding comment about changes to {scope_id} case"
                        )
                        try:
                        ...
                        try:
                            self.soar_job.add_comment(
                                comment=format_text(
                                    ...
                                    prefix=SYNC_INCIDENTS_JOB_COMMENT_PREFIX,
                                    incident_number=incident_number,
                                    object_key=related_objects_mapping.get(
                                        link, {}
                                    ).get("key"),
                                    name=related_objects_mapping.get(link, {}).get(
                                        "value"
                                    ),
                                    key=key,
                                    old=change[0],
        ...
        for sys_id, values in self._cached_affected_cis_mapping.items():
            self._updated_incident_affected_cis.get("removed", []).extend(
                [
                    ...
                    for number in list(
                        set(values.get("incident_numbers", [])).difference(
                            self._affected_cis_mapping.get(sys_id, {}).get(
                                "incident_numbers", []
                            )
                        )
                    )
    ...
    def add_affected_cis_changes_comments(self):
        ...
        for key, items in self._updated_incident_affected_cis.items():
            for incident_number, affected_ci_sys_id in items:
                for scope_id in incidents_mapping.get(incident_number, []):
                    self.logger.info(
                        f"Adding incident affected CI changes comment to "
                        f"{scope_id} case"
                    )
                    try:
                        self.soar_job.add_comment(
                            comment=format_text(
                                ...
                                prefix=SYNC_INCIDENTS_JOB_COMMENT_PREFIX,
                                incident_number=incident_number,
                                name=affected_cis.get(affected_ci_sys_id, {}).get(
                                    "name"
                                ),
                                key=key,
                            ),
        ...
        for sys_id, changes in self._affected_cis_changes.items():
            for key, change in changes.items():
                for incident_number in affected_cis_mapping.get(sys_id, {}).get(
                    "incident_numbers"
                ):
                    for scope_id in incidents_mapping.get(incident_number, []):
                        self.logger.info(
                            f"Adding affected CI changes comment to {scope_id} case"
                        )
                        try:
                            self.soar_job.add_comment(
                                comment=format_text(
        ...
        try:
            ...
            if isinstance(wall_activities, dict) and "activities" in wall_activities:
                for item in wall_activities.get("activities", []):
                    ...
                    if item.get("activityType") == "CaseComment" and activity_data.get(
                        "Type"
                    ):
                        if (
                            activity_data.get("Name", "").startswith(
                                SNOW_ATTACHMENT_PREFIX
                            )
                ...
                for item in wall_activities:
                    if item.get("type") == CASE_ATTACHMENT_TYPE and item.get(
                        "fileType"
                    ):
                        if (
                            item.get("evidenceName", "").startswith(
                                SNOW_ATTACHMENT_PREFIX
                            )
                            or item.get("modificationTimeUnixTimeInMsForClient")
                            < timestamp
                        ):
                            continue
            ...
            self.logger.info(
                f"Failed to get attachments for case {case_id}. Reason: {e}"
            )
    ...
    def sync_utilities_snow_siem(self, datetime):
        ...
        for incident_raw_data in self._updated_incidents_data.values():
            ...
            self.sync_attachments_snow_siem(
                incident_raw_data, sn_format_datetime
            )
    ...
    def sync_attachments_snow_siem(self, incident_raw_data, datetime):
        ...
        """
        self.logger.info(
            f'Getting incident {incident_raw_data.get("number")} attachments'
        )
        ...
        for incident_attachment in incident_attachments:
            ...
            try:
                ...
                self.logger.error(
                    f"Failed to get {incident_attachment.filename} "
                    f"file content. Reason: {err}"
                )
                continue
            ...
            for scope_id in self._cached_incident_numbers_mapping.get(
                ...
                try:
                    self.add_attachment_to_case(
                        scope_id,
                        encoded_content,
                        f"{SNOW_ATTACHMENT_PREFIX} " f"{incident_attachment.filename}",
                    )
                    self.logger.info(
                        f"Successfully added new attachment to {scope_id} case"
                    )
    ...
    def sync_comments_snow_siem(self, incident_raw_data, datetime):
        ...
        try:
            self.logger.info(
                f'Getting incident {incident_raw_data.get("number")} comments'
            )
            ...
            self.logger.info(
                f'Comments for incident {incident_raw_data.get("number")} not found '
                f"in ServiceNow"
            )
            incident_comments = []
        ...
        for comment in filtered_case_comments:
            for scope_id in self._cached_incident_numbers_mapping.get(
                ...
                try:
                    ...
                    self.logger.info(
                        f"Failed to add comment to {scope_id} case. Reason: {e}"
                    )
                ...
                try:
                    if case_id in case_ids:
                        self.api_client.add_attachment_to_incident(
                            self._cached_incidents_data.get(incident_number).get(
                                "sys_id"
                            ),
                            file_name,
                            attachment_content,
    ...
    def filter_comments_by_prefix(comments, key, prefixes_to_except):
        ...
        return [
            ...
            for comment in comments
            if comment.get(key, "")
            and not comment.get(key, "").startswith(prefixes_to_except)
        ]
    ...
    def set_cache_data(self):
        ...
        set_chunks_as_job_context_property(
            ...
            INCIDENTS_KEY,
            split_dict_into_chunks(
                incidents, chunk_size=INCIDENTS_CONTEXT_VALUE_CHUNK_SIZE
            ),
        )
    ...
    def _perform_job(self) -> None:
        ...
        if new_incident_numbers_to_cache:
            ...
            self.logger.info(
                f"Fetched {len(self._incidents_data)} incidents data from ServiceNow"
            )
        ...
        self.logger.info(
            "Getting incident related objects data from ServiceNow to cache"
        )
        for link in self._related_objects_mapping:
            self._related_objects[link] = self.get_related_object(link)

        self.logger.info(
            f"Fetched {len(self._related_objects)} related objects from ServiceNow"
        )

        self.logger.info(
            "Getting updated related objects data from ServiceNow to compare"
        )
        for key in self._cached_related_objects_mapping.keys():
            self._updated_related_objects_data[key] = self.get_related_object(
                key
            )
        ...
        self.logger.info(
            f"Fetched {len(self._updated_affected_cis)} updated Affected CIs "
            f"from ServiceNow"
        )
        ...
        if self._related_objects_changes:
            self.logger.info(
                "Adding comments about changes in related objects to cases"
            )
