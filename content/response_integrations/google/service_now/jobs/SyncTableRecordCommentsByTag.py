...
def main():
    ...
    password = extract_action_param(
        siemplify=siemplify, param_name="Password", is_mandatory=True
    )
    ...
    try:
        ...
        open_cases = {
            str(case_id): get_full_case_details(siemplify, case_id)
        ...
        siemplify.LOGGER.info(
            "--- Start synchronize comments ServiceNow <-> Siemplify ---"
        )
        ...
        for case_id, case in open_cases.items():
            ...
            if record_ids:
                ...
                try:
                    ...
                    new_comments_from_case, new_comments_from_sn = (
                        get_new_comments_to_add(raw_case_comments, raw_sn_comments)
                    )
                    ...
                    for sn_comment in new_comments_from_sn:
                        try:
                            ...
                            siemplify.LOGGER.info(
                                f"Added comment to case with id: {case_id}"
                            )
                        except Exception as e:
                            siemplify.LOGGER.error(
        ...
        siemplify.LOGGER.info(
            "--- Finish synchronize comments ServiceNow <-> Siemplify ---"
        )
