...
def main(is_first_run):
    ...
    statuses_list = (
        [status.strip() for status in statuses.lower().split(",")] if statuses else []
    )
    ...
    try:
        ...
        if ticket.state.lower() in statuses_list:
            ...
            output_message = f"Continuing...waiting for incident {incident_number} status to be updated"
        ...
        output_message = f'Error executing action "{WAIT_FOR_STATUS_UPDATE_SCRIPT_NAME}". Reason: {e}'
        result_value = False
