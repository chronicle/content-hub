...
from ..core.constants import CLOSE_INCIDENT_SCRIPT_NAME, INTEGRATION_NAME
...
def main():
    ...
    close_reason = extract_action_param(
        siemplify, param_name="Close Reason", print_value=True, is_mandatory=True
    )

    close_notes = extract_action_param(
        siemplify, param_name="Close Notes", print_value=True, is_mandatory=True
    )

    close_code = extract_action_param(
        siemplify, param_name="Resolution Code", print_value=True, is_mandatory=True
    )
    ...
    try:
        ...
        service_now_manager.close_incident(
            ticket_number=incident_number,
            close_reason=close_reason,
            close_code=close_code,
            close_notes=close_notes,
        )
        output_message = f"Successfully closed incident with number {incident_number}."
        ...
        output_message = (
            f"General error performing action {CLOSE_INCIDENT_SCRIPT_NAME}. Reason: {e}"
        )
        result_value = False
