...
from ..core.constants import INTEGRATION_NAME, ADD_COMMENT_AND_WAIT_FOR_REPLY_SCRIPT_NAME
...
def main():
    ...
    try:
        ...
        output_message = (
            f"Comment {comment_to_add} was posted at: {last_comment_creation_time}"
        )
...
def query_job():
    ...
    try:
        ...
        for comment in comments_list:
            if parser.parse(comment.sys_created_on) > parser.parse(
                last_comment_creation_time
            ):
        ...
        if new_comment:
            ...
            output_message = f"Continuing...waiting for new comment to be added to {incident_number} incident"
