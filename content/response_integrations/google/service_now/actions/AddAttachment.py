...
def main():
    ...
    try:
        ...
        service_now_manager.is_exists_in_table(
            sys_id=record_sys_id, table_name=table_name
        )
            ...
            try:
                if mode == OVERWRITE_EXISTING_ATTACHMENT:
                    ...
                    for existing_attachment in existing_attachments:
                        service_now_manager.delete_attachment(
                            sys_id=existing_attachment.sys_id
                        )
        ...
        output_message = (
            f'General error performing action "Add Attachment". Reason: {e}'
        )
        result_value = False
