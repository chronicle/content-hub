...
from .product import Phishrod


class PhishrodSession(
    MockSession[MockRequest, MockResponse, Phishrod]
):
    ...
    @router.get(r".*/api/jsonws/phishrod\.psc_secondary_analysis_tracking/get\-secondary\-analysis\-tracking\?apiKey=\S+\&clientId=\S+")
    ...
    @router.get(r".*/api/jsonws/phishrod\.psc_primary_analysis_tracking/get\-primary\-analysis\-tracking\?apiKey=\S+\&clientId=\S+")
    def get_incidents(self, request: MockRequest) -> MockResponse:
        try:
            response_data = self._product.get_incidents()
            return MockResponse(content=response_data)
        except Exception as e:
            return MockResponse(content=str(e), status_code=500)

    @router.get(r".*/api/jsonws/phishrod\.psc_secondary_analysis_tracking/update\-incident\-status\?apiKey=\S+\&clientId=\S+\&incidentNumber=\S+\&incidentStatus=\S+")
    def update_incident(self, request: MockRequest) -> MockResponse:
        try:
            response_data = self._product.update_incident()
            return MockResponse(content=response_data)
        except Exception as e:
            return MockResponse(content=str(e), status_code=500)

    @router.get(r".*/api/jsonws/phishrod\.psc_primary_analysis_tracking/initiate\-primary\-analysis\?apiKey=\S+\&clientId=\S+\&incidentNumber=\S+\&action=\S+\&comment=\S+")
    def mark_incident(self, request: MockRequest) -> MockResponse:
        try:
            response_data = self._product.mark_incident()
            return MockResponse(content=response_data)
        except Exception as e:
            return MockResponse(content=str(e), status_code=500)
