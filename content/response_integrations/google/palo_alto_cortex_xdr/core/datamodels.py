...
from soar_sdk.SiemplifyUtils import add_prefix_to_dict, dict_to_flat
...
class Alert(BaseModel):
    ...
    def to_json(self) -> SingleJson:
        ...
        if not self.raw_data.get(API_HOST_NAME_KEY):
            endpoint_data = self.raw_data.get(API_ENDPOINT_DATA_KEY) or self.raw_data.get(API_ENDPOINT_KEY)
            if isinstance(endpoint_data, dict):
                host_name = endpoint_data.get(API_HOST_NAME_KEY) or endpoint_data.get(API_ENDPOINT_NAME_KEY)
                if host_name:
