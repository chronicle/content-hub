...
from ..core.constants import INTEGRATION_NAME, CREATE_CASE_SCRIPT_NAME
...
def main():
    ...
    try:
        ...
        case = manager.create_case(
            name=name, priority=priority, due_date=due_date, description=description
        )
        output_message = (
            f"Successfully created case {case.number} in {INTEGRATION_NAME}."
        )
        siemplify.result.add_result_json(case.as_json())

    except Exception as e:
        output_message = (
            f"Error executing action {CREATE_CASE_SCRIPT_NAME}. Reason: {e}"
        )
