...
def main():
    ...
    is_send_email_deactivate = (
        siemplify.parameters.get("Send Email If Deactivate", "false").lower() == "true"
    )
    ...
    if ids:
        for _id in ids:
            try:
                ...
                if res:
                    message += f"The user with id {_id} was disabled ({action}).\n\n"
                    ret[_id] = action
                else:
                    message += (
                        f"The user with id {_id} couldn't be disabled ({action}).\n\n"
                    )
                    continue
            except Exception as err:
                siemplify.LOGGER.exception(err)
                siemplify.LOGGER.error(f"{_id} {action}: {err}")
                errors += f"{str(err)}\n\n"
                pass
    if is_scope:
        entitiesDisabled = []
        for entity in siemplify.target_entities:
            if (
                ...
                try:
                    res = okta.disable_user(
                        entity.identifier, is_deactivate, is_send_email_deactivate
                    )
                    if res:
                        entitiesDisabled.append(entity)
                        message += f'The user "{entity.identifier}" was disabled ({action}).\n\n'
                        ret[entity.identifier] = action
                    else:
                        message += f'The user "{entity.identifier}" couldn\'t be disabled ({action}).\n\n'
                        continue
                except Exception as err:
                    siemplify.LOGGER.exception(err)
                    siemplify.LOGGER.error(
                        f"{entity.identifier} {action}: {err}"
                    )
                    errors += f"{str(err)}\n\n"
                    pass
            else:
    ...
    if ret:
        ...
        output_message = f"No users were disabled. {message}"
