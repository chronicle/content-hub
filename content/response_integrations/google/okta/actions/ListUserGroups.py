...
def main():
        ...
        for _id in ids:
            try:
                ...
                if res:
                    ...
                    message += f"The user with id {_id} belongs to {len(groups)} groups: {', '.join(groups)}\n\n"
                else:
            ...
            if (
                ...
                try:
                    ...
                    if res:
                        ...
                        message += (
                            f'The user "{entity.identifier}" belongs to '
                            f"{len(groups)} groups: {', '.join(groups)}\n\n"
                        )
                    else:
                        message += f'The user "{entity.identifier}" doen\'t belong to any group.\n\n'
                        continue
                except Exception as err:
                    siemplify.LOGGER.exception(err)
    ...
    if ret:
        ...
        for user, groups in list(ret.items()):
            rows = []
            if groups:
                for group in groups:
                    if group:
            ...
            if rows:
                ...
                siemplify.result.add_data_table(
                    f'Okta - User "{user}" Groups', csv_output
                )
        if not flag:
