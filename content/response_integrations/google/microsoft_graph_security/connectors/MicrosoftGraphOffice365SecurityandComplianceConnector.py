...
from TIPCommon.transformation import convert_comma_separated_to_list, dict_to_flat
...
O365_V2_PROVIDER_NAME = "microsoftDefenderForOffice365"
...
def main(is_test_run):
    ...
    provider_list = [O365_PROVIDER_NAME] if not use_v2_api else [O365_V2_PROVIDER_NAME]
    ...
    for alert in fetched_alerts:
        try:
            siemplify.LOGGER.info(
                f"Started processing Alert {alert.id} - {alert.title or ALERT_NAME}",
                alert_id=alert.id,
            )
            ...
            if not pass_severity_filter(siemplify, alert, alert_severities_to_fetch):
                siemplify.LOGGER.info(
                    f"Alert {alert.id} did not pass severity filter. Skipping...."
                )
                continue

            if not pass_status_filter(siemplify, alert, alert_statuses_to_fetch or []):
                siemplify.LOGGER.info(
                    f"Alert {alert.id} did not pass status filter. Skipping...."
                )
                continue
            ...
            if overflowed and not is_test_run:
                siemplify.LOGGER.info(f"Alert {alert.id} is overflow. Skipping")
                continue
            ...
            siemplify.LOGGER.error(
                f"Could not add alert '{alert.title}' ({alert.id}) to Siemplify. Skipping."
            )
...
def pass_severity_filter(siemplify, alert, severity_filter):
    # severity filter
    if severity_filter and alert.severity not in convert_comma_separated_to_list(
        severity_filter
    ):
        siemplify.LOGGER.info(
            f"Alert with severity: {alert.severity} did not pass filter. Acceptable severities are: {severity_filter}."
        )
        return False
    return True
...
def pass_status_filter(siemplify, alert, status_filter):
    ...
    if api_status_filter and alert.status not in api_status_filter:
        siemplify.LOGGER.info(
            f'Alert with status: {API_TO_UI_STATUSES_MAPPING.get(alert.status, "N/A")} did not pass filter. Acceptable statuses are: {status_filter}.'
        )
        return False
    return True
