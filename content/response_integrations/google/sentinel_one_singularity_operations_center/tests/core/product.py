...
class SentinelOne:
    users: list[dict] = dataclasses.field(
        default_factory=lambda: [
            {"id": "98765", "email": "analyst@company.com", "fullName": "Mock Analyst", "username": "mock.analyst"}
        ]
    )
    ...
    details: dict[str, dict] = dataclasses.field(
        default_factory=lambda: {
            "019d114e-e4f4-7ad6-82c3-9829b6d0a801": {
                ...
                "analytics": {"category": "Reputation", "name": "Agent Policy", "typeValue": "DYNAMIC", "uid": None},
                "assignee": None,
                "assets": [
                    {
                ...
                "observables": [{"name": "process.name", "type": "PROCESS", "typeName": None, "value": "avm.exe"}],
                "process": {
                    "cmdLine": "avm.exe",
    ...
    def get_unified_alerts(self, first: int, view_type: str, after: str | None = None) -> SingleJson:
    ...
    def trigger_actions(self, actions: list[dict], alert_filter: dict) -> SingleJson:
            ...
            return {
                "data": {
                    "alertTriggerActions": {
                        ...
                        "errors": [{"errorMessage": f"Alert with ID {alert_id} not found.", "errorPayload": None}],
                    }
                }
            }
        ...
        for action in actions:
            ...
            elif action_id == "S1/alert/analystVerdictUpdate" and "analystVerdict" in payload:
        ...
        return {"data": {"alertTriggerActions": {"__typename": "ActionsTriggered", "actions": action_results}}}

    def get_users(self, email: str | None = None) -> dict:
        """Mock method to return a list of users filtered by email."""
        filtered_users = [u for u in self.users if u["email"].lower() == email.lower()] if email else self.users
    ...
    def add_alert_note(
        ...
        new_note = {
            ...
            "author": {"id": "98765", "email": "analyst@company.com", "fullName": "Mock Analyst"},
        }
