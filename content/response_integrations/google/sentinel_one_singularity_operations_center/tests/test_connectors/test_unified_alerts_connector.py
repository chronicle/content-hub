...
def test_successful_connector_run(
    ...
    assert alert.display_id == "SentinelOne_Singularity_Operations_Center_019d114e-e4f4-7ad6-82c3-9829b6d0a801"
    assert alert.priority == 100  # CRITICAL maps to 100
    assert alert.rule_generator == "SentinelOne Singularity Operations Center Alert: MALWARE"
...
def test_classification_blocklist_filter(
    ...
    monkeypatch.setattr(type(connector.siemplify), "whitelist", property(lambda _: ["MALWARE"]))
...
def test_classification_allowlist_filter(
    ...
    # Mock the read-only whitelist property on the SDK mock
    monkeypatch.setattr(type(connector.siemplify), "whitelist", property(lambda _: ["PHISHING"]))
...
def test_id_cache_cleanup_optimization(
    ...
    new_details = copy.deepcopy(sentinelone.details["019d114e-e4f4-7ad6-82c3-9829b6d0a801"])
...
@set_metadata(
    connector_def_file_path=DEF_PATH,
    parameters={**DEFAULT_PARAMETERS, "Max Alerts To Fetch": 110, "Disable Overflow": "True"},
)
def test_connector_pagination(
    ...
    p1_vars = common.get_request_payload(script_session.request_history[0].request)["variables"]
    ...
    # Note: Because the generator streams alerts, 100 detail queries are interleaved
    # between Page 1 and Page 2 list queries.
    p2_vars = common.get_request_payload(script_session.request_history[101].request)["variables"]
...
def test_alert_updates_supported(
    ...
    external_context.set_row_value(
        ...
        identifier=None,
        property_key="ids",
        property_value=json.dumps({"019d114e-e4f4-7ad6-82c3-9829b6d0a801": old_timestamp_ms}),
    )

    # Modify mock alert's list and details updatedAt to be newer (15 minutes ago)
    new_timestamp_str = arrow.utcnow().shift(minutes=-15).format("YYYY-MM-DDTHH:mm:ss.SSS") + "Z"
