...
from .constants import EVENT_TYPE_FIELD, INTEGRATION_DISPLAY_NAME, LAST_SEEN_AT_FIELD, SEVERITY_MAP, EventType
...
class SentinelOneAlert(BaseAlert):
    ...
    def __init__(self, raw_data: dict, details: SentinelOneAlertDetails | None = None) -> None:
        ...
        *,
        ...
        if is_update:
            ...
            alert_info.updated_fields = {
                ...
                "analystVerdict": self.details.analyst_verdict if self.details else None,
                "severity": self.severity,
            }

        alert_info.ticket_id = self.alert_id
        alert_info.display_id = f"SentinelOne_Singularity_Operations_Center_{self.alert_id}"
        ...
        alert_info.rule_generator = f"SentinelOne Singularity Operations Center Alert: {classification}"
        alert_info.source_grouping_identifier = classification

        # Timestamp parsing
        created_at_str = self.created_at
        dt_start = dateutil.parser.parse(created_at_str) if created_at_str else last_success_timestamp
    ...
    def from_api_response(
        ...
        if typename == "ActionsTriggered":
            ...
            for action in actions:
                ...
                failures.extend(f"{action['actionId']}: {f.get('errorMessage')}" for f in action.get("failure") or [])
                skips.extend(f"{action['actionId']}: {s.get('skipMessage')}" for s in action.get("skip") or [])
        ...
        if typename == "TriggerActionsError":
            ...
            error_msgs = [e.get("errorMessage") for e in errors if e.get("errorMessage")]
