...
from ..data_models import AlertUpdateResult, SentinelOneAlertDetails
...
class SentinelOneSingularityOperationsCenterApiClient(Apiable):
    ...
    @staticmethod
    def _build_unified_alerts_or_filter(
        ...
        if lowest_severity:
            ...
            if lowest_severity_upper in SEVERITIES:
                ...
                common_filters.append({"fieldId": "severity", "stringIn": {"values": target_severities}})
        ...
        branch_last_seen = {
            "and": [
                {"fieldId": "lastSeenAt", "dateTimeRange": {"start": start_timestamp_ms, "end": None}},
                *common_filters,
            ]
        }
        branch_updated = {
            "and": [
                {"fieldId": "updatedAt", "dateTimeRange": {"start": start_timestamp_ms, "end": None}},
                *common_filters,
            ]
        }

        return {"or": [branch_last_seen, branch_updated]}
    ...
    def yield_unified_alerts(
        ...
        or_filter = self._build_unified_alerts_or_filter(
        ...
        nodes_gen = (self._extract_node(edge) for edges, _ in pages_gen for edge in edges)
    ...
    def update_alert(
        ...
        if status:
            actions_payload.append({"id": "S1/alert/statusUpdate", "payload": {"status": {"value": status}}})
        ...
        if assignee_id is not False:
            actions_payload.append({"id": "S1/alert/assignUser", "payload": {"assignUser": {"value": assignee_id}}})
    ...
    def add_alert_comment(
        ...
        Returns:
            dict: The raw mutation response data.
