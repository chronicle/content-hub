...
from ..core.api.api_client import ApiParameters, SentinelOneSingularityOperationsCenterApiClient
...
CONNECTOR_SCRIPT_NAME = "SentinelOne Singularity Operations Center - Unified Alerts Connector"
MAX_CACHE_SIZE = 1100
...
class UnifiedAlertsConnector(Connector):
    ...
    def validate_params(self) -> None:
        ...
        self.params.use_dynamic_list_as_a_blocklist = str(self.params.use_dynamic_list_as_a_blocklist).lower() == "true"
        self.params.disable_overflow = str(self.params.disable_overflow).lower() == "true"
    ...
    def read_context_data(self) -> None:
        ...
        raw_ids = read_content(self.siemplify, IDS_FILE_NAME, IDS_DB_KEY, default_value_to_return={})

        self.logger.info("Loaded raw context: %s", raw_ids)
    ...
    def store_alert_in_cache(self, processed_alert: SentinelOneAlert) -> None:
        ...
        self.context.existing_ids[processed_alert.alert_id] = convert_datetime_to_unix_time(
            dateutil.parser.parse(processed_alert.updated_at)
        )
        if len(self.context.existing_ids) > MAX_CACHE_SIZE:
            self.logger.info(
                "Cache size (%s) exceeded limit (%s). Pruning oldest entries down to %s.",
                len(self.context.existing_ids),
                MAX_CACHE_SIZE,
                PRUNED_CACHE_SIZE,
            )
            self.context.existing_ids = {
                key: self.context.existing_ids[key]
                for key in sorted(self.context.existing_ids, key=self.context.existing_ids.get)[-PRUNED_CACHE_SIZE:]
            }
    ...
    def set_last_success_time(self, alerts: list[SentinelOneAlert]) -> None:
        """Save the updated success timestamp."""
    ...
    def get_alerts(self) -> list[SentinelOneAlert]:
        ...
        last_run_success_time_ms = convert_datetime_to_unix_time(self.context.last_success_timestamp)

        # Get latest cached alert timestamp
        latest_cached_ms = max(self.context.existing_ids.values()) if self.context.existing_ids else 0

        # Start from the newer of the two
        start_timestamp_ms = max(last_run_success_time_ms, latest_cached_ms)

        self.logger.info(
            "Fetching up to %s unified alerts. Last run success time: %s ms. "
            "Latest cached alert time: %s ms. Start timestamp: %s ms.",
            self.params.max_alerts_to_fetch,
            last_run_success_time_ms,
            latest_cached_ms,
            start_timestamp_ms,
        )
        ...
        for raw_alert in alerts_generator:
            # Check timeout before enriching and adding
            ...
            enriched_alerts.append(self._get_alert_details(SentinelOneAlert(raw_alert)))

        return enriched_alerts

    def filter_alerts(self, fetched_alerts: list[SentinelOneAlert]) -> list[SentinelOneAlert]:
        """Filter out already processed alerts unless they have been updated.

        Args:
            fetched_alerts (list[SentinelOneAlert]): The list of alerts fetched from the API.

        Returns:
            list[SentinelOneAlert]: The filtered list of alerts.

        """
        filtered_alerts = []
        for alert in fetched_alerts:
            alert_updated_ms = convert_datetime_to_unix_time(dateutil.parser.parse(alert.updated_at))

            # Ingest if brand new, or if its update timestamp is newer than our cache
            cached_time = self.context.existing_ids.get(alert.alert_id)
            if cached_time is None or alert_updated_ms > cached_time:
                filtered_alerts.append(alert)
            else:
                self.logger.info("Alert %s skipped. No new updates since last fetch.", alert.alert_id)

        return filtered_alerts

    def _get_alert_details(self, alert: SentinelOneAlert) -> SentinelOneAlert:
        """Get and set the alert details from the API.

        Args:
            alert (SentinelOneAlert): The alert to enrich.

        Returns:
            SentinelOneAlert: The enriched alert with details set.

        """
        if not alert.is_detailed:
    ...
    def create_alert_info(self, processed_alert: SentinelOneAlert) -> AlertInfo:
        ...
        """
        flat_alert = dict_to_flat(processed_alert.details.raw_data if processed_alert.details else {})
