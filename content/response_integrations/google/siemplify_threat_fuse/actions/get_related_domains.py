...
def main():
    ...
    max_domains_to_return = extract_action_param(
        siemplify,
        param_name="Max Domains To Return",
        is_mandatory=False,
        print_value=True,
        default_value=consts.MAX_DOMAINS_TO_RETURN_DEFAULT,
        input_type=int,
    )
    ...
    try:
        if (
            confidence_threshold > consts.MAX_CONFIDENCE
            or confidence_threshold < consts.MIN_CONFIDENCE
        ):
        ...
        for association_type in search_association_types:
            ...
            if get_max_dict_value_size(json_results) >= max_domains_to_return:
                siemplify.LOGGER.info(
                    f"Json results size reached domains max size of {max_domains_to_return}"
                )
                break
            ...
            try:
                ...
                for association in related_associations:
                    ...
                    query_limit = max_domains_to_return - get_max_dict_value_size(
                        json_results
                    )
                ...
                siemplify.LOGGER.error(
                    f"An error occurred when querying association type {association_type}"
                )
        ...
        if get_max_dict_value_size(json_results) > 0:
            siemplify.LOGGER.info(
                f"Successfully retrieved related domains from {INTEGRATION_NAME}"
            )
            output_message = (
                f"Successfully retrieved related domains from {INTEGRATION_NAME}"
            )
