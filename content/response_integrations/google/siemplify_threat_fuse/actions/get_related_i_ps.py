...
def main():
    ...
    max_ips_to_return = extract_action_param(
        siemplify,
        param_name="Max IPs To Return",
        is_mandatory=False,
        print_value=True,
        default_value=consts.MAX_IPS_TO_RETURN_DEFAULT,
        input_type=int,
    )
    ...
    try:
        ...
        for association_type in search_association_types:
            ...
            if get_max_dict_value_size(json_results) >= max_ips_to_return:
                siemplify.LOGGER.info(
                    f"Json results size reached ips max size of {max_ips_to_return}"
                )
                break
            ...
            try:
                ...
                for association in related_associations:
                    ...
                    query_limit = max_ips_to_return - get_max_dict_value_size(
                        json_results
                    )
        ...
        if get_max_dict_value_size(json_results) > 0:
            siemplify.LOGGER.info(
                f"Successfully retrieved related IPs from {INTEGRATION_NAME}"
            )
            output_message = (
                f"Successfully retrieved related IPs from {INTEGRATION_NAME}"
            )
