...
def main():
    ...
    max_email_addresses_to_return = extract_action_param(
        siemplify,
        param_name="Max Email Addresses To Return",
        is_mandatory=False,
        print_value=True,
    ...
    try:
        ...
        for association_type in search_association_types:
            ...
            try:
                ...
                for association in related_associations:
                    if (
                        get_max_dict_value_size(json_results)
                        >= max_email_addresses_to_return
                    ):
                        siemplify.LOGGER.info(
        ...
        for entity in actors_and_vulnerabilities:
            ...
            try:
                ...
                query_limit = max_email_addresses_to_return - get_max_dict_value_size(
                    json_results
                )
                association_indicators = manager.get_association_type_indicators(
        ...
        if get_max_dict_value_size(json_results) > 0:
            siemplify.LOGGER.info(
                f"Successfully retrieved related email addresses from {INTEGRATION_NAME}"
            )
