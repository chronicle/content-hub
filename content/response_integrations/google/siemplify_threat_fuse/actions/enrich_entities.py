...
def main():
    ...
    ignore_false_positive = extract_action_param(
        siemplify,
    ...
    try:
        ...
        numeric_severity_threshold = consts.SEVERITIES_ORDER[
            consts.SEVERITIES[severity_threshold]
        ]
        ...
        if supported_entities:
            try:
                ...
                indicators = manager.get_indicators(
                    entities=[
                        entity.identifier.strip() for entity in supported_entities
                    ]
                )
        ...
        if indicators:  # check if retrieved indicators
            ...
            for indicator_group in indicator_groups:
                try:
                    ...
                    siemplify.LOGGER.info(
                        f"Processing indicators of entity {indicator_group.entity.identifier}"
                    )
                    is_suspicious = False
                    ...
                    json_results["results"][indicator_group.entity.identifier] = [
                        indicator.as_json() for indicator in indicator_group.indicators
                    ]
                    ...
                    siemplify.LOGGER.info(
                        f"Fetching analysis links for entity {indicator_group.entity.identifier}"
                    )
                    ...
                    if create_insight and (
                        ...
                        try:
                            if (
                                ...
                                siemplify.LOGGER.info(
                                    f"Fetching intel details for {indicator_group.entity.identifier}"
                                )
                                ...
                                siemplify.LOGGER.info(
                                    f"Adding insight for entity {indicator_group.entity.identifier}"
                                )
                    ...
                    siemplify.LOGGER.error(
                        f"An error occurred on entity {indicator_group.entity.identifier}"
                    )
                    siemplify.LOGGER.exception(e)
                    failed_entities.append(indicator_group.entity)
