...
from ..core.constants import (
    ...
    GET_RELATED_ASSOCIATION_SCRIPT_NAME,
    EMAIL_TYPE,
    ENDPOINTS_MAPPER,
    ...
    MAX_ASSOCIATIONS_TO_RETURN_DEFAULT,
    MAX_STATISTICS_TO_RETURN_DEFAULT,
    NOT_ASSIGNED,
)
from ..core.utils import (
    append_association_type_to_entities,
    get_entity_type,
...
def continue_operation(
    ...
    create_actors_entity,
    create_signature_entity,
    create_vulnerability_entity,
    create_insight,
    create_case_tag,
    stats_limit,
):
    ...
    num_unprocessed_ids = sum(
        len(association_types[association_type].get("unprocessed_ids", []))
        for association_type in association_types.keys()
    )

    if num_unprocessed_ids > 0:  # continue fetching details
        siemplify.LOGGER.info(
            f"Fetching total of {num_unprocessed_ids} unprocessed related associations."
        )
        ...
        for association_type in association_types.keys():
            ...
            unprocessed_ids = (
                json_results["association_type"][association_type]
                .get("unprocessed_ids", [])
                .copy()
            )

            if (
                len(unprocessed_ids) <= 0
            ):  # Check if new related association needs to be fetched
                continue

            siemplify.LOGGER.info(
                f"Fetching {len(unprocessed_ids)} related association of type "
                f"{association_type}"
            )
            ...
            for association_id in unprocessed_ids:
                ...
                try:
                    ...
                    siemplify.LOGGER.info(
                        f"Retrieving details for {association_type} with id "
                        f"{association_id}"
                    )
                    ...
                    if stats_limit != 0:
                        if association_details:
                            try:
                                association_indicators_json = (
                                    manager.get_association_type_indicators_stats(
                                        association_type=ENDPOINTS_MAPPER[
                                            association_type
                                        ],
                                        association_id=association_id,
                                        limit=stats_limit,
                                ...
                                siemplify.LOGGER.info(
                                    f"Coudn't fetch IOC details for {association_type} "
                                    f"with ID {association_id}. Reason: {e}"
                                )
                                continue
                    else:
                    ...
                    siemplify.LOGGER.info(
                        f"Successfully retrieved details for {association_type} with "
                        f"id {association_id}"
                    )
                    ...
                    json_results["association_type"][association_type][
                        "json_results"
                    ].append(association_details_json)
                    ...
                    siemplify.LOGGER.info(
                        f"Failed to fetch details for association {association_type} "
                        f"with id {association_id}"
                    )
            ...
            if unprocessed_ids:
                siemplify.LOGGER.info(
                    f"Processed {processed} {association_type} details"
                )
                json_results[
                    "total_processed"
                ] += processed  # update total processed associations
        ...
        if csv_table:
            siemplify.result.add_data_table(
                title=f"Related Associations", data_table=construct_csv(csv_table)
            )

        # Create final json result for the action
        final_json_results = defaultdict(list)
        for association_type in association_types.keys():
            final_json_results[ENDPOINTS_MAPPER[association_type]].extend(
                association_types[association_type].get("json_results", [])
            )
        ...
        if (
            create_insight and successfully_processed > 0
        ):  # Create insight if successfully retrieved details
            insight_title = f"Found {len(csv_table)} related associations"
            insight_content = ""
            for association_type in association_types.keys():
                insight_content += "".join(
                    association_types[association_type].get("insights", [])
                )
        ...
        ):  # check if successfully retrieved some related association details
            result_value = True
            output_message = (
...
def start_operation(
    ...
    entity_identifiers = [
        get_entity_original_identifier(entity) for entity in supported_entities
    ]
    siemplify.LOGGER.info(
    ...
    if indicator_ids:
        ...
        for association_type, value in search_association_types.items():
            ...
            try:
                ...
                siemplify.LOGGER.info(
                    f"Successfully received {len(related_associations)} related "
                    f"association ids of type {association_type}"
                )
                if related_associations:
                    associations_to_process += len(related_associations)
                    result_value["association_type"][association_type][
                        "unprocessed_ids"
                ...
                siemplify.LOGGER.info(
                    "Failed to get related associations for association type "
                    f"{association_type}"
                )
        ...
        if associations_to_process > 0:  # check if some related association was found
            siemplify.LOGGER.info(
                f"Received total of {associations_to_process} related associations to "
                "fetch"
            )
            status = EXECUTION_STATE_INPROGRESS
            result_value = json.dumps(result_value)
            output_message = (
                "Waiting for all of the association details to be retrieved"
            )
        else:
            status = EXECUTION_STATE_COMPLETED
            result_value = False
        ...
        siemplify.LOGGER.info(
            "Didn't get indicators for supported entities "
            f"{', '.join(entity_identifiers)}"
        )
        status = EXECUTION_STATE_COMPLETED
        result_value = False
        output_message = "No related associations were found."
...
def main(is_first_run):
    ...
    create_case_tag = extract_action_param(
        siemplify,
        param_name="Create Case Tag",
        input_type=bool,
        print_value=True,
        default_value=False,
    )
    ...
    if max_associations_to_return < 0:
        siemplify.LOGGER.info(
            f'"Max Associations To Return" must be non-negative. '
    ...
    try:
        max_stats_to_return = extract_action_param(
            siemplify,
            param_name="Max Statistics To Return",
            print_value=True,
            input_type=int,
        )
        ...
        if max_stats_to_return and max_stats_to_return < 0:
            siemplify.LOGGER.info(
                '"Max Statistics To Return" must be non-negative. Using default of '
                f'{MAX_STATISTICS_TO_RETURN_DEFAULT}.'
            )
        ...
        if is_first_run:
            ...
            output_message, result_value, status = continue_operation(
                siemplify=siemplify,
                manager=manager,
        ...
        output_message = (
            f'Error executing action "{GET_RELATED_ASSOCIATION_SCRIPT_NAME}". '
            f'Reason: {error}'
        )
