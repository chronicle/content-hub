...
from .product import UrlScanIo


class UrlScanIoSession(
    MockSession[MockRequest, MockResponse, UrlScanIo]
):
    ...
    @router.post(r".*api/v1/scan")
    def scan(self, request: MockRequest) -> MockResponse:
        try:
            response_data = self._product.submit_url_for_scan()
            return MockResponse(content=response_data)
        except Exception as e:
            return MockResponse(content=str(e), status_code=500)

    @router.get(r".*api/v1/search")
    def search(self, request: MockRequest) -> MockResponse:
        try:
            response_data = self._product.search_scans()
            return MockResponse(content=response_data)
        except Exception as e:
            return MockResponse(content=str(e), status_code=500)

    @router.get(r".*api/v1/result/\S+")
    def result(self, request: MockRequest) -> MockResponse:
        try:
            response_data = self._product.get_url_scan_report()
            return MockResponse(content=response_data)
        except Exception as e:
            return MockResponse(content=str(e), status_code=500)
