...
from TIPCommon.extraction import extract_configuration_param, extract_action_param; from TIPCommon.transformation import construct_csv
...
def main():
    ...
    try:
        ...
        if not successful_entities:
            output_message = (
                "Action wasn’t able to list scans for the available entities."
            )
            result_value = False
        ...
        output_message = (
            f"Error executing action '{SEARCH_FOR_SCANS_SCRIPT_NAME}'. Reason: {err}"
        )
