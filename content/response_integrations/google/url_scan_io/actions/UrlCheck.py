...
def query_operation_status(siemplify, manager, scan_report, suitable_entities):
    ...
    for entity, scan_id in scan_report["in_progress"].items():
        try:
            ...
            siemplify.LOGGER.error(
                f"An error occurred when checking status for url {entity}"
            )
            siemplify.LOGGER.exception(err)
...
def finish_operation(
    ...
    for entity in suitable_entities:
        if get_entity_original_identifier(entity) in completed_entities.keys():
            ...
            shorten_json = entity_result.to_shorten_json()
            shorten_json["is_risky"] = int(entity_result.score) >= threshold
            json_results[get_entity_original_identifier(entity)] = shorten_json
            ...
            try:
                if entity_result.screenshot_url:
                    ...
                    base64_screenshot = get_screenshot_content_base64(
                        screenshot_content
                    )
                    siemplify.result.add_attachment(
                        title=SCREENSHOT_TITLE.format(
                            get_entity_original_identifier(entity)
                        ),
                ...
                if only_suspicious_insight:
                    if int(entity_result.score) >= threshold:
                        siemplify.create_case_insight(
                            ...
                            title="URL Details",
                            content=entity_result.as_url_insight(
                                screenshot_to_add=screenshot
                            ),
...
def main(is_first_run):
    ...
    try:
        ...
        output_message = (
            f"General error performing action {URL_CHECK_ACTION_NAME} Reason: {err}"
        )
        result_value = False
