...
from ..core import ZerofoxManager as api_manager


class AddEvidenceToAlert(Action):

    def __init__(self) -> None:
        super().__init__(constants.ADD_EVIDENCE_TO_ALERT_SCRIPT_NAME)
        self.output_message: str = ""
        self.result_value: bool = False
        self.error_output_message: str = (
            f'Error executing action "{constants.ADD_EVIDENCE_TO_ALERT_SCRIPT_NAME}".'
        )
    ...
    def _extract_action_parameters(self) -> None:
        ...
        self.params.filepath: str = extract_action_param(
            self.soar_action,
            param_name="Filepath",
            is_mandatory=True,
            print_value=True,
    ...
    def _perform_action(self, _) -> None:
        try:
            self.api_client.add_evidence_to_alert(
                alert_id=self.params.alert_id,
                evidence_path=self.params.filepath,
            )
            self.result_value = True
            self.output_message = (
                "Successfully added evidence to the alert with ID "
