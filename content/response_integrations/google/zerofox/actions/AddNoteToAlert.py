...
class AddNoteToAlert(Action):

    def __init__(self) -> None:
        super().__init__(constants.ADD_NOTE_TO_ALERT_SCRIPT_NAME)
        self.output_message: str = ""
        self.result_value: bool = False
        self.error_output_message: str = (
            f'Error executing action "{constants.ADD_NOTE_TO_ALERT_SCRIPT_NAME}".'
        )
    ...
    def _extract_action_parameters(self) -> None:
        ...
        self.params.note: str = extract_action_param(
            self.soar_action,
            param_name="Note",
            is_mandatory=True,
            print_value=True,
    ...
    def _perform_action(self, _) -> None:
        try:
            self.api_client.add_note_to_alert(
                alert_id=self.params.alert_id,
                note=self.params.note,
            )
            self.result_value = True
            self.output_message = (
                "Successfully added a note to the alert with ID "
