    ...
    try:
        ...
        if is_first_run:
            if containment_status == "contained":
                ...
                is_success = True
                output_message = (
                    f"Endpoint '{agent_id}' is already contained in SentinelOne."
                )
                json_result["operation_results"][agent_id]["result"] = "success"
                json_result["operation_results"][agent_id]["status"] = "contained"
                json_result["operation_results"][agent_id]["reason"] = None
                siemplify.LOGGER.info(output_message)
            elif containment_status == "containment_requested":
                action_status = EXECUTION_STATE_INPROGRESS
                is_success = True
                output_message = (
                    f"Waiting for containment to finish for endpoint '{agent_id}'."
                )
                json_result["operation_results"][agent_id]["result"] = "success"
                ...
                siemplify.LOGGER.info(
                    f"Starting containment for agent '{agent_id}' (ID: {found_agent.id})"
                )
                manager.disconnect_agent_from_network(found_agent.id)
                action_status = EXECUTION_STATE_INPROGRESS
                is_success = True
                output_message = f"Containment initiated for endpoint '{agent_id}'. Waiting for containment to finish."
                json_result["operation_results"][agent_id]["result"] = "success"
                json_result["operation_results"][agent_id]["status"] = (
                    "containment_requested"
                )
            ...
            if containment_status == "contained":
                ...
                is_success = True
                output_message = (
                    f"Successfully contained endpoint '{agent_id}' in SentinelOne."
                )
                json_result["operation_results"][agent_id]["result"] = "success"
                json_result["operation_results"][agent_id]["status"] = "contained"
                json_result["operation_results"][agent_id]["reason"] = None
        ...
        if agent_id:
            if agent_id not in json_result["operation_results"]:
                json_result["operation_results"][agent_id] = {"operation": "contain"}
        ...
        timeout_message = f"Containment request timed out for endpoint '{agent_id}'."
