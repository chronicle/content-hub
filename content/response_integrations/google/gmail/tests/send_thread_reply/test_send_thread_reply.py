...
from gmail.actions.SendThreadReply import (
    SendThreadReply,
    SUCCESS_MESSAGE,
    MESSAGE_NOT_FOUND,
)
...
ERROR_PREFIX = (
    f"Error executing action \"{Constants.SEND_THREAD_REPLY_SCRIPT_NAME}\"\nReason:"
)
...
ACTION_CONFIG_WITH_COMMA_SEPARATED_PARAMS["Reply To"] = (
...
ACTION_CONFIG_WITH_ATTACHMENTS["Attachments Paths"] = (
    ",".join(
        ...
        for file_name in (
            f"thread_{ATTACHMENT_NAME}",
            f"thread_{ATTACHMENT_NAME_WITH_UNKNOWN_TYPE}"
        )
    )
)
...
class TestAuth:
    ...
    def test_without_creds(
        ...
        SendThreadReply(script_name=Constants.SEND_THREAD_REPLY_SCRIPT_NAME).run()
...
class TestSendEmail:
    ...
    def test_with_no_params(
        ...
        action = SendThreadReply(script_name=Constants.SEND_THREAD_REPLY_SCRIPT_NAME)
        action.run()
        ...
        assert_all_send_message(gmail_script_session.request_history, start=2, stop=3)
    ...
    def test_with_attachments_in_original_email(
        ...
        google_gmail.set_messages(
            MOCK_DATA["mail_without_attachments"] + MOCK_DATA["mail_with_files"]
        )
        ...
        assert_all_send_message(gmail_script_session.request_history, start=4, stop=5)
        assert_all_get_message(gmail_script_session.request_history, start=5)
