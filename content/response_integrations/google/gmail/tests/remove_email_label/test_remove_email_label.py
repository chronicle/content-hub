...
ERROR = (
    f"Error executing action \"{Constants.REMOVE_EMAIL_LABEL_SCRIPT_NAME}\"\nReason:"
)
...
ACTION_CONFIG_WITH_MIXED_LABELS = ACTION_CONFIG_WITH_MAILBOXES.copy()
ACTION_CONFIG_WITH_MIXED_LABELS["Label"] += ",invalid"
...
class TestAuth:
    ...
    def test_without_creds(
        ...
        action = RemoveEmailLabel(script_name=Constants.REMOVE_EMAIL_LABEL_SCRIPT_NAME)
        action.run()
...
class TestFilter:
    ...
    def test_invalid_label(
        ...
        assert action_output.results.output_message == (
            ALL_LABELS_INVALID.format("test@mailbox.com")
        )
    ...
    @set_metadata(
        integration_config=CONFIG,
        parameters=ACTION_CONFIG_WITH_MIXED_LABELS
    )
    def test_mixed_labels(
        ...
        assert action_output.results.output_message == (
            SUCCESS_MESSAGE.format(2, "test@mailbox.com") +
            LABELS_INVALID.format("test@mailbox.com", "invalid")
        )
        assert action_output.results.result_value is True
        assert action_output.results.execution_state == ExecutionState.COMPLETED
