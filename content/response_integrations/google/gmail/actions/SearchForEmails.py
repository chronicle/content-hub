...
from TIPCommon.transformation import construct_csv, convert_dict_to_json_result_dict
from TIPCommon.validation import ParameterValidator

from ..core.GoogleGmailBaseAction import GoogleGmailBaseAction
from ..core.GoogleGmailConsts import (
    REQUIRED_HEADERS,
    SEARCH_FOR_EMAILS_SCRIPT_NAME,
)
...
NO_FOUND_MESSAGES = (
    "No emails found in the following mailboxes:\n{}\n\n"
)
PENDING_MESSAGE = (
    "Pending search for emails in the following mailboxes:\n{}\n"
)
UNFINISHED_MAILBOX_MESSAGE = (
    "The action wasn't able to complete retrieving emails in the following mailboxes:"
    "\n{}\nIncrease the timeout in IDE and try again.\n\n"
)
...
class SearchForEmails(GoogleGmailBaseAction):
    ...
    def _extract_action_parameters(self) -> None:
        ...
        self.params.recipient_filter = extract_action_param(
            self.soar_action,
            "Recipient Filter",
            default_value=""
        )
        ...
        self.params.headers_to_return = extract_action_param(
            self.soar_action,
            "Headers To Return"
        )
        self.params.return_email_body = extract_action_param(
            self.soar_action,
            "Return Email Body",
            input_type=bool
        )
        self.params.max_emails_to_return = extract_action_param(
            self.soar_action,
    ...
    def _validate_params(self) -> None:
        ...
        self.params.labels_list = validator.validate_csv(
            "Labels",
            self.params.labels,
        )
        ...
        self.params.headers_list = validator.validate_csv(
            "Headers to Return",
            self.params.headers_to_return
        )
        if self.params.headers_list:
            self.params.headers_list.extend(
                set(REQUIRED_HEADERS).difference(self.params.headers_list)
            )

        self.params.emails_limit = validator.validate_positive(
            "Max Emails To Return",
            self.params.max_emails_to_return,
        )
        ...
        if self.params.recipient_filter:
            self.params.recipient_filter = validator.validate_email(
                "Recipient Filter",
                self.params.recipient_filter
            )
    ...
    def _prepare_statistics_message(self) -> str:
        ...
        none_found = []
        json_results = {}
        message = NO_MESSAGES_FOUND_MESSAGE

        for mailbox_info in self.action_context.processed_mailboxes:
            if not mailbox_info.found_emails:
                none_found.append(mailbox_info.mailbox_name)
                continue

            some_found.append(
                f"Fetched {mailbox_info.finished_emails} out of "
        ...
        if some_found:
            ...
            self.data_tables.append(
                DataTable(
                    ...
                    data_table=construct_csv([
                        GmailMessage.csv_from_json(message_json, mailbox_name)
                        for mailbox_name, messages in json_results.items()
            ...
            if none_found:
                message += NO_FOUND_MESSAGES.format(", ".join(none_found))
    ...
    async def _get_message_ids(self, messages_service: MessagesService) -> list[str]:
        ...
        return await messages_service.search_messages(
            ...
            recipient_filter=self.params.recipient_filter,
            limit=self.params.emails_limit,
    ...
    async def _process_mailbox(self, mailbox_name: str) -> None:
        try:
            ...
            async with api_client.session:
                ...
                fetch_message_tasks = coros_to_tasks_with_limit(
                    (
                        messages_service.get_message_by_id(
                            message_id,
                            format_=(
                                "full" if self.params.return_email_body
                                else "metadata"
                            ),
                            metadata_headers=self.params.headers_list
                        )
                ...
                try:
                    ...
                    results = tuple(
                        {
                            **result.to_json(),
                            **result.payload.to_json(self.params.headers_list)
                        }
                        if isinstance(result, GmailMessage) else result
                        for result in messages
                    )
                    self.logger.info(
                        f"Finished {len(results)} fetch operations for mailbox "
                        f"{mailbox_name}"
                    )
                    self.action_context.mark_mailbox_as_finished(
                        mailbox_name,
                        results,
                        overwrite=True
                    )
                    ...
                    finished_tasks_results = (
                        task.result() for task in fetch_message_tasks
                    ...
                    results = tuple(
                        ...
                        if isinstance(result, GmailMessage) else result
                        for result in finished_tasks_results
                    )
...
def main() -> None:
    SearchForEmails(SEARCH_FOR_EMAILS_SCRIPT_NAME).run()
