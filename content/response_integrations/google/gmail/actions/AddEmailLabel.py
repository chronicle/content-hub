...
class AddEmailLabel(GoogleGmailBaseAction):
    ...
    async def _perform_action_async(self, _=None) -> None:
        async with self.api_client.session:
            ...
            for label_name in self.params.labels_to_add:
                ...
                if label_ is None:
                    label_ = await labels_service.create_label(label_name)
            ...
            await messages_service.add_labels(
                message_ids,
...
def main() -> None:
    AddEmailLabel(ADD_EMAIL_LABEL_SCRIPT_NAME).run()
