...
from TIPCommon.smp_io import read_ids, write_ids_with_timestamp
...
def main(is_test_run):
    ...
    try:
        ...
        if fetch_limit < 1:
            siemplify.LOGGER.info(
                f"Max Detections To Fetch must be greater than zero. The default value {DEFAULT_LIMIT} "
                f"will be used"
            )
            fetch_limit = DEFAULT_LIMIT

        if lowest_risk_to_fetch and (
            lowest_risk_to_fetch < 0 or lowest_risk_to_fetch > 100
        ):
            raise Exception(
                f"Invalid value given for Lowest Risk Score To Fetch parameter. "
            ...
            try:
                ...
                for participant in alert.participants:
                    if participant.get("object_type", "") == DEVICE_OBJECT_TYPE:
                        alert.devices.append(
                            manager.get_device_details(
        ...
        if not is_test_run:
            ...
            save_timestamp(
                siemplify=siemplify, alerts=fetched_alerts, timestamp_key="update_time"
            )
