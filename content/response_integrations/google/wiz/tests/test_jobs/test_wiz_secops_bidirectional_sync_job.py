...
class TestWizSecopsBidirectionalSyncJob:
    ...
    def test_sync_severity_escalation_comment_deduplication(self) -> None:
        """Verifies that multiple alerts with the same threat ID only add a single escalation comment."""
        job = _make_job()
        alerts = [
            _make_mock_alert("alert_1", "wiz_threat_1", priority="Medium"),
            _make_mock_alert("alert_2", "wiz_threat_1", priority="Medium"),
            _make_mock_alert("alert_3", "wiz_threat_1", priority="Medium"),
        ]
        case = _make_mock_case(1234, alerts)
        job_case = JobCase(case_detail=case, modification_time=1000)
        job_case.alert_metadata["alert_1"] = SyncMetadata(severity="CRITICAL")
        job_case.alert_metadata["alert_2"] = SyncMetadata(severity="CRITICAL")
        job_case.alert_metadata["alert_3"] = SyncMetadata(severity="CRITICAL")

        job.sync_severity_to_case = MagicMock()

        job.sync_severity(job_case)

        assert job.sync_severity_to_case.call_count == 3
        job.soar_job.add_comment.assert_called_once()
        assert "Severity Escalation" in job.soar_job.add_comment.call_args[1]["comment"]
    ...
    def test_sync_comments_loop_back_prevention(self) -> None:
        ...
        alerts = [_make_mock_alert("alert_1", "wiz_threat_1")]
        comments = [
            {"comment": "[Wiz Note] wiz_threat_1: note from Wiz", "creator": "sync@soar.com"},
