...
from ..core.FireEyeHelixConstants import PROVIDER_NAME, CLOSE_ALERT_SCRIPT_NAME
...
def main():
    ...
    revision_note = extract_action_param(
        siemplify, param_name="Revision Note", is_mandatory=False
    )
    ...
    try:
        ...
        output_message = (
            f"Successfully closed the alert with ID {alert_id} in FireEye Helix."
        )

    except FireEyeHelixNotFoundAlertException:
        output_message = (
            "Action wasn't able to close the alert with ID {id} in FireEye Helix. "
