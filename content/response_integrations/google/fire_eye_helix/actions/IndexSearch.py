...
from ..core.FireEyeHelixConstants import PROVIDER_NAME, INDEX_SEARCH_SCRIPT_NAME
...
def main():
    ...
    try:
        ...
        result = manager.index_search(query=query, time_frame=time_frame, limit=limit)

        if result.contains_results():
            siemplify.result.add_result_json(result.to_json())
            output_message = f'Successfully returned results for the query "{query}" in {PROVIDER_NAME}.'
        else:
            output_message = f'No results were found for the query "{query}".'
            result_value = False

    except Exception as e:
        output_message = f'Error executing action "Index Search". Reason: {e}'
