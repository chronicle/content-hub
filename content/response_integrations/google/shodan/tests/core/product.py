...
class Shodan:
    ...
    _get_ip_info_response: dict | None = None
    ...
    def set_get_ip_info_response(self, response: dict) -> None:
        self._get_ip_info_response = response

    def get_ip_info(self, *args, **kwargs) -> dict:
        if self._fail_requests_active:
            raise Exception("Simulated API failure for get_ip_info")
        if self._get_ip_info_response is not None:
            return self._get_ip_info_response
        return {
            "ok": True,
            "data": {"id": "mock_id"},
            "results": [{"id": "mock_id"}],
