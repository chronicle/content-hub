...
ERROR_MESSAGE = f'Error executing action "{Constants.EXECUTE_PROMPT_SCRIPT_NAME}"'
...
ACTION_CONFIG_WITH_TOKEN_COUNT = ACTION_CONFIG.copy()
ACTION_CONFIG_WITH_TOKEN_COUNT["Max Input Tokens"] = 5
...
class TestValid:
    @set_metadata(integration_config=CONFIG, parameters=ACTION_CONFIG_WITH_TOKEN_COUNT)
    ...
    @set_metadata(integration_config=CONFIG, parameters=ACTION_CONFIG)
    def test_valid_run(
        self,
        vertexai_script_session: ApiSession,
        action_output: MockActionOutput,
