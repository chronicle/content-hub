...
from ..core.constants import INTEGRATION_NAME, UNBLOCK_URLS_SCRIPT_NAME
from ..core.exceptions import CategoryNotExistsException
...
def main():
    ...
    siemplify.script_name = UNBLOCK_URLS_SCRIPT_NAME
    ...
    policy_name = extract_action_param(
        siemplify, param_name="URL Category Name", print_value=True
    )
    ...
    try:
        ...
        if not use_shared_objects:
            ...
            urls_from_category = api.ListSharedUrlsFromCategory(
                category_name=policy_name
            )
            not_existing_entities = [
                url
                for url in suitable_entity_identifiers
                if url not in urls_from_category
            ]
            stay_urls = [
                url
                for url in urls_from_category
                if url not in suitable_entity_identifiers
            ]
            removable_entities = [
                url for url in urls_from_category if url in suitable_entity_identifiers
            ]
        ...
        if successful_entities:
            output_message = (
                "Successfully removed the following URLs from the shared category '{}' in "
                "Palo Alto NGFW: {} \n".format(
                    policy_name, ", ".join(successful_entities)
                )
            )
        else:
            output_message = f"No URLs were removed from the shared category '{policy_name}' in Palo Alto NGFW.\n"
            result_value = False

        if not_existing_entities:
            result_value = True
            output_message += (
                "The following URLs were not a part of the the shared category '{}' "
                "in Palo Alto NGFW: {}\n".format(
                    policy_name, ", ".join(not_existing_entities)
                )
            )
        ...
        output_message = f'Error executing action "Unblock URLs". Reason: {e}'
