...
class CyberXManager:
    ...
    def get_device_by_host_name(self, host_name):
        ...
        for device in devices:
            if host_name.lower() == device.get("name").lower():
                return device
        raise CyberXManagerError(f'Error, No device with host name "{host_name}" was found.')
    ...
    def get_device_id_by_address(self, ip_address):
        ...
        raise CyberXManagerError(f'Error: No ID found for device with address "{ip_address}"')
    ...
    def get_device_id_by_host_name(self, host_name):
        ...
        if device_information.get("id"):
            return device_information.get("id")
        raise CyberXManagerError(f'Error: No ID found for device for host "{host_name}"')
    ...
    def get_devices_vulnerability_reports(self):
        ...
        """
        request_url = urllib.parse.urljoin(self.api_root, GET_DEVICES_VULNERABILITIES_REPORT_URL)
        response = self.session.get(request_url)
        self.validate_response(response)
        return response.json()
