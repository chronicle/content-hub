...
class SendVoteEmail(Action[api_manager.ApiManager]):

    def __init__(self) -> None:
        super().__init__(constants.SEND_VOTE_EMAIL_NAME)
        self.output_message = ""
        self.error_output_message = (
            f'Error executing action "{constants.SEND_VOTE_EMAIL_NAME}".'
        )
        self.json_results = {}
    ...
    def _extract_action_parameters(self) -> None:
        ...
        self.params.voting_options = extract_action_param(
            self.soar_action,
            param_name="Structure of voting options",
            default_value=constants.DEFAULT_VOTING_OPTION,
            is_mandatory=True,
            print_value=True,
        )
    ...
    def _send_email(self) -> MicrosoftGraphEmail:
        if EmailUtils.is_rtl(self.params.email_html_template):
            self.params.email_html_template = EmailUtils.add_rtl_html_divs_to_body(
                self.params.email_html_template
            )
        ...
        send_mail = self.api_client.send_vote_html_email(
            ...
            voting_option=self.params.voting_options,
        )
