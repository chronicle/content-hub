...
def main():
        ...
        try:
            ...
            if computer_id:
                siemplify.LOGGER.info(
                    f"Disabling DownloadInsight on {entity.identifier}"
                )
            ...
            errors += (
                f"Disabling Download Insight failed on {entity.identifier}:\n{e}\n"
            )
    ...
    if enriched_entities:
        ...
        output_message += (
            f"Disabling Download Insight for {time_period} minutes on the following entities:\n"
            + "\n".join(entities_names)
        )
