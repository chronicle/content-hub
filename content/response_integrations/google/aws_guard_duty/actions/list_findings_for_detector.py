...
from ..core.consts import ASC, DEFAULT_MAX_RESULTS, INTEGRATION_NAME
...
def _run_action(siemplify: SiemplifyAction, config: AWSGuardDutyConfig) -> tuple[str, str]:
    ...
    if max_results_to_return and max_results_to_return < 0:
        ...
        siemplify.LOGGER.info(
            f"Max Trusted IP Lists To Return parameter was non-positive. Using default value of {DEFAULT_MAX_RESULTS}"
        )
...
@output_handler
def main() -> None:
    """List all Amazon GuardDuty findings for the specified detector."""
