...
def _run_action(siemplify: SiemplifyAction, config: AWSGuardDutyConfig) -> tuple[str, str]:
    ...
    not_unarchived_findings = [finding_id for finding_id in findings_ids if finding_id not in fetched_findings_ids]
    ...
    for finding in fetched_findings_ids:
        try:
            ...
            siemplify.LOGGER.exception(f"Action wasn't able to un-archive Finding {finding}")
