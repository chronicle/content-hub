...
def _run_action(siemplify: SiemplifyAction, config: AWSGuardDutyConfig) -> tuple[str, str]:
    ...
    if max_results_to_return and max_results_to_return < 0:
        ...
        siemplify.LOGGER.info(
            "Max Threat Intelligence Sets To Return parameter was non-positive. "
            f"Using default value of {DEFAULT_MAX_RESULTS}"
        )
    ...
    ti_sets_ids = manager.get_threat_intelligence_sets_ids(detector_id=detector_id, max_results=max_results_to_return)
    siemplify.LOGGER.info(f"Successfully found {len(ti_sets_ids)} threat intelligence sets ids.")
    output_message = "Successfully listed available Threat Intelligence Sets."
