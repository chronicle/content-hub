...
def main():
    ...
    try:
        ...
        if members == members_not_assigned:
            output_message = (
                f"No members were assigned to the {group_assigned.get('name')} group. "
            ...
            if members_not_assigned:
                output_message = (
                    f"Successfully assigned '{len(members_assigned)}' members to the "
                    f"{group_assigned.get('name')} group. However, the following members "
                    f"were not assigned: {', '.join(members_not_assigned)}. Please provide "
                    f"valid members of type {group_type} that exists."
                )
            else:
                output_message = (
                    f"Successfully assigned {len(members)} members to the "
