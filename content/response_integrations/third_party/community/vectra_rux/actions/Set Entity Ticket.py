...
def main():
    ...
    siemplify.script_name = SET_ENTITY_TICKET_SCRIPT_NAME
    ...
    try:
        ...
        entity_ticket = vectra_manager.set_entity_ticket(entity_type, entity_id, external_reference_id)

        output_message = (
            f"The external reference ID for provided Entity ID have been "
            f"updated as {external_reference_id}"
        )
        
        siemplify.result.add_result_json(json.dumps(entity_ticket, indent=4))
