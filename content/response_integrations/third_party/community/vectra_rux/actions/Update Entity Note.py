...
from ..core.constants import (
    ...
    RESULT_VALUE_FALSE,
    RESULT_VALUE_TRUE,
    UPDATE_ENTITY_NOTE_SCRIPT_NAME,
)
...
def main():
    ...
    siemplify.script_name = UPDATE_ENTITY_NOTE_SCRIPT_NAME
    ...
    note_id = siemplify.extract_action_param(
        param_name="Note ID",
        input_type=str,
        is_mandatory=True,
    )
    ...
    try:
        ...
        # Updating the Note of the given Entity ID
        updated_note = vectra_manager.update_entity_note(entity_type, entity_id, note_id, note)
        
        siemplify.result.add_result_json(
            json.dumps(updated_note.raw_data, indent=4),
        )
        
        siemplify.result.add_data_table(
                title="Update Entity Notes",
                data_table=updated_note.update_entity_notes_csv(),
            )

        output_message = "The note has been successfully updated in the entity"
