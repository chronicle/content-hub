...
from actions.RemoveIpFromBlockList import main
...
@patch.object(SignalSciencesManager, "remove_ip_from_blocklist")
def test_remove_ip_from_block_list_success(
    mock_remove_ip, mock_get_block, script_session, action_output
):
    ...
    assert (
        "Successfully removed the following IPs from the Block List for "
...
def test_remove_ip_from_block_list_not_found(
    ...
    mock_get_block.return_value = [] # IP not in blocklist
...
@patch.object(SignalSciencesManager, "get_blocklists")
@patch.object(SignalSciencesManager, "remove_ip_from_blocklist")
def test_remove_ip_from_block_list_duplicates(
    mock_remove_ip, mock_get_block, script_session, action_output
):
    mock_get_block.return_value = [
...
@patch.object(SignalSciencesManager, "get_blocklists")
def test_remove_ip_from_block_list_no_ips(
...
def test_remove_ip_from_block_list_site_not_found(
    ...
    assert 'Error executing action: "Remove IP from Block List". Reason: Site non-existent-site not found.' in action_output.results.output_message
    assert action_output.results.is_success is False
...
@patch.object(SignalSciencesManager, "get_blocklists")
@patch.object(SignalSciencesManager, "remove_ip_from_blocklist")
def test_remove_ip_from_block_list_full_failure(
    mock_remove_ip, mock_get_block, script_session, action_output
):
    mock_get_block.return_value = [{"id": "id1", "source": "1.2.3.4"}]
    mock_remove_ip.side_effect = Exception("API Error")

    main()

    assert 'Error executing action: "Remove IP from Block List". Reason: Failed to remove' in action_output.results.output_message
    assert action_output.results.is_success is False
    assert action_output.results.execution_state == ExecutionState.FAILED
