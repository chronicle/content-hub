...
class TestRemoveIpFromAllowList:
    ...
    def test_remove_ip_success(
        ...
        # Arrange
        signal_sciences.add_whitelist_item("test_site", {"source": "1.1.1.1", "id": "wl_1"})
        ...
        assert get_request.url.path.endswith("/whitelist")

        delete_request = script_session.request_history[1].request
        assert delete_request.method.value == "DELETE"
        assert delete_request.url.path.endswith("/whitelist/wl_1")
