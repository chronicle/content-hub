...
def _load_payload(action_key: str) -> dict[str, Any]:
    payloads_directory = INTEGRATION_ROOT / "dev" / "payloads"
    payload_path = payloads_directory / f"{_pascal_to_snake(action_key)}.json"
    if not payload_path.exists():
        print(f"Payload file not found at {payload_path}. Using empty payload.")
        return {}
...
def run_action(action_name: str, payload: dict[str, Any]) -> None:
    ...
    with patch(
        "TIPCommon.base.action.base_action.create_soar_action", return_value=stub_soar
    ):
        action = action_class(script_name)
        action.run()
    print(f"Action '{action_name}' completed.")
    print(f"output_message: {action.output_message}")
    if action.json_results:
        print(f"json_result: {json.dumps(action.json_results, indent=2)}")
...
def main() -> None:
    ...
    try:
        ...
        print(f"Runner error: unsupported action '{args.action}' ({exc})")
        raise exc
