...
SCRIPT_NAME = "Create Tool"


class CreateToolParameters(BaseActionParameters):
    ...
    description: str | None = Field(
        description="Specify tool description",
        default=None,
    )
    tool_types: list[str] | None = Field(
        description="Specify tool types",
        default=None,
    )
    ...
    marking: str | None = Field(
        description="Specify marking for the tool",
        default=None,
    )
...
class CreateTool(BaseAction):
    ...
    @property
    def params(self) -> CreateToolParameters:
    ...
    def _validate_params(self) -> None:
        ...
        self._params = CreateToolParameters(  # type: ignore[assignment]
            name=raw_params.get("Name"),
            description=raw_params.get("Description"),
            tool_types=raw_params.get("Tool Types"),
            labels=raw_params.get("Labels"),
            marking=raw_params.get("Marking"),
        )

    def _perform_action(self, _=None) -> None:
        """Create a Tool entity in OpenCTI using the input values.
        On success it records the created tool details in output fields;
        on failure an exception propagates and the framework marks the action as failed.
        """
        tool = Tool(
            name=self.params.name,
            description=self.params.description,
            tool_types=self.params.tool_types,
            labels=self.params.labels,
            markings=[self.params.marking] if self.params.marking else None,
        )
        result = self.api_client.create_tool(tool)

        self.output_message = f"Tool successfully created with ID {result.id}"
