"""Mark Case Not An Attack action for Abnormal Security Google SecOps SOAR Integration."""
...
@output_handler
def main() -> None:
    """Main execution logic for the Mark Case Not An Attack action."""
    ...
    try:
        ...
        response = manager.post_case_action(
            case_id=case_id,
            action="acknowledge_not_an_attack",
        )
        siemplify.result.add_result_json(response)
        output_message = f"Case {case_id} acknowledged as not an attack."
        result_value = True
        ...
        output_message = (
            f'Error executing action "{MARK_CASE_NOT_AN_ATTACK_SCRIPT_NAME}". Reason: {e}'
        )
