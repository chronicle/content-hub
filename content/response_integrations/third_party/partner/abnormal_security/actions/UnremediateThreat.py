...
def main() -> None:
    ...
    try:
        ...
        if successes and not failures:
            output_message = f"UnremediateThreat succeeded for {len(successes)} threat(s): {', '.join(successes)}."
            result_value = True
            status = EXECUTION_STATE_COMPLETED
        elif successes and failures:
            output_message = f"UnremediateThreat: {len(successes)} succeeded, {len(failures)} failed. " + "; ".join(
                f"{t}: {e}" for t, e in failures
            )
            result_value = True
            status = EXECUTION_STATE_COMPLETED
        else:
            output_message = f"UnremediateThreat failed for all {len(failures)} threat(s): " + "; ".join(
                f"{t}: {e}" for t, e in failures
            )
