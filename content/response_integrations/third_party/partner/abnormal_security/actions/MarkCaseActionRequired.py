...
def main() -> None:
    ...
    try:
        ...
        if successes and not failures:
            output_message = f"MarkCaseActionRequired succeeded for {len(successes)} case(s): {', '.join(successes)}."
            result_value = True
            status = EXECUTION_STATE_COMPLETED
        elif successes and failures:
            output_message = (
                f"MarkCaseActionRequired: {len(successes)} succeeded, {len(failures)} failed. "
                + "; ".join(f"{c}: {e}" for c, e in failures)
            )
            result_value = True
            status = EXECUTION_STATE_COMPLETED
        else:
            output_message = f"MarkCaseActionRequired failed for all {len(failures)} case(s): " + "; ".join(
