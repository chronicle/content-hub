"""Resolve Case action for Abnormal Security Google SecOps SOAR Integration."""
...
@output_handler
def main() -> None:
    """Main execution logic for the Resolve Case action."""
    ...
    try:
        ...
        response = manager.post_case_action(
            case_id=case_id,
            action="acknowledge_resolved",
        )
        siemplify.result.add_result_json(response)
        output_message = f"Case {case_id} acknowledged as resolved."
        result_value = True
        ...
        output_message = (
            f'Error executing action "{RESOLVE_CASE_SCRIPT_NAME}". Reason: {e}'
        )
