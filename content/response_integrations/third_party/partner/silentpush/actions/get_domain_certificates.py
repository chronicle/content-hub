...
from ..core.constants import GET_DOMAIN_CERTIFICATES_SCRIPT_NAME, INTEGRATION_NAME
...
def main():
    ...
    try:
        ...
        raw_response = sp_manager.get_domain_certificates(
            domain, **(filter_params or {})
        )
        ...
        if raw_response.get("response", {}).get("job_status", {}):
            ...
            return siemplify.end(output_message, result_value, status)
        ...
        output_message = (
            f"domain: {domain}, certificates: {certificates}, metadata: {metadata}"
        )
        status = EXECUTION_STATE_COMPLETED
        result_value = True
        ...
        status = EXECUTION_STATE_FAILED
        output_message = (
            f"Failed to retrieve certificates data for domain {domain} "
