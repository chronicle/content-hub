...
def main():
    ...
    try:
        ...
        raw_response = sp_manager.search_scan_data(query, limit=limit, fields=fields, skip=skip, with_metadata=with_metadata)
        ...
        result_value = False
        status = EXECUTION_STATE_FAILED
        output_message = f"Failed to retrieve scan data records, for {INTEGRATION_NAME} server! Error: {error}"
