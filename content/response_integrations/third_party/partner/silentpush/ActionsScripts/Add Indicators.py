...
from constants import (INTEGRATION_NAME, ADD_INDICATORS_SCRIPT_NAME )
...
def main():
    ...
    indicators = siemplify.extract_action_param("indicators", print_value=True)
    
    try:
        sp_manager = SilentPushManager(server_url, api_key, logger=siemplify.LOGGER)
        result = sp_manager.add_indicators(feed_uuid=feed_uuid,indicators=indicators)
