...
from constants import (INTEGRATION_NAME, FORWARD_PADNS_LOOKUP_SCRIPT_NAME )
...
def main():
    ...
    siemplify.script_name = FORWARD_PADNS_LOOKUP_SCRIPT_NAME 
    ...
    first_seen_after = siemplify.extract_action_param("first_seen_after", print_value=True)
    first_seen_before = siemplify.extract_action_param("first_seen_before", print_value=True)
    last_seen_after = siemplify.extract_action_param("last_seen_after", print_value=True)
    last_seen_before = siemplify.extract_action_param("last_seen_before", print_value=True)
    ...
    try:
        ...
        raw_response = sp_manager.forward_padns_lookup(
                        qtype=qtype,
                        qname=qname,
                        netmask=netmask,
