...
class MockEntity:
    ...
    def __init__(
        self, identifier: str, entity_type: str, additional_properties: dict
    ):
...
class CensysAPIManager:
    ...
    def enrich_hosts(
        self, host_ids: list, at_time: Optional[str] = None
    ) -> dict:
    ...
    def initiate_rescan(
        ...
        if self.should_fail_initiate_rescan:
            self._raise_exception()
        return self.initiate_rescan_response or {
            "result": {"scan_id": "mock_scan_id"}
        }
    ...
    def get_rescan_status(self, scan_id: str) -> dict:
        ...
        if self.should_fail_rescan_status:
            self._raise_exception()
        return self.rescan_status_response or {
    ...
    class MockSession:
        ...
        def request(self, method, url, **kwargs):
            ...
            if (
                ...
                "/scans/rescan" in url
                and censys_manager.should_fail_initiate_rescan
            ):
                censys_manager._raise_exception()
            elif (
                "/scans/" in url
            ...
            if "/accounts/organizations/" in url:
                ...
                return MockResponse(
                    censys_manager.enrich_web_properties_response
                    or {"result": []},
                    200,
                )
            elif "/asset/certificate" in url:
                return MockResponse(
                    censys_manager.enrich_certificates_response
                    or {"result": []},
                    200,
                )
            elif "/timeline" in url:
                return MockResponse(
                    censys_manager.host_history_response
                    or {"result": {"events": []}},
                    200,
                )
    ...
    def _convert_dict_to_entity(entity_dict: dict) -> MockEntity:
        ...
        return MockEntity(
            ...
            additional_properties=dict(
                entity_dict.get("additional_properties", {})
            ),
        )
