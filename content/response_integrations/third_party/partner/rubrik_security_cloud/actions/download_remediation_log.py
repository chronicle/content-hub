...
from ..core.constants import (
    ...
    DOWNLOAD_REMEDIATION_LOG_SCRIPT_NAME,
    FILE_STATE_FAILED,
    FILE_STATE_READY,
    RESULT_VALUE_FALSE,
    RESULT_VALUE_TRUE,
...
ACTION_ERROR_PREFIX = "Failed to generate remediation log"

# RSC names remediation log downloads as "<object_name> actions log...".
...
REMEDIATION_LOG_FILENAME_SUFFIX = " actions log"


@output_handler
def main(is_first_run):
    siemplify = SiemplifyAction()
    siemplify.script_name = DOWNLOAD_REMEDIATION_LOG_SCRIPT_NAME
    ...
    try:
        ...
        if is_first_run:
            ...
            # object but a different violation.
            ...
            # Violation Details call supplies the resourceId and object name used for
            # these checks.
            ...
            # Step 2: trigger remediation log generation (only after inputs validated).
            siemplify.LOGGER.info("Step 2: Creating violation remediation log.")
            remediation_id = manager.trigger_remediation_log(
                violation_id=violation_id,
                resource_id=object_id,
            )
            siemplify.LOGGER.info(f"Step 2 complete: remediationId={remediation_id}")

            # Step 3: immediately look up the resulting file's externalId via
            # allUserFiles, matched by object name + file type + recency.
            target_file_name = f"{object_name}{REMEDIATION_LOG_FILENAME_SUFFIX}"

            siemplify.LOGGER.info(
                f"Step 3: Locating triggered remediation log (matching filename "
                f"containing '{target_file_name}')."
            )
            file_entry = manager.find_user_file_by_filename(
                file_type=USER_FILE_TYPE_LOG,
            ...
            output_message = "Waiting for remediation log to be generated..."
            status = EXECUTION_STATE_INPROGRESS
            ...
            if file_state == FILE_STATE_READY:
                filename = file_entry.get("filename") or "remediation_log.csv"
                siemplify.LOGGER.info(
                ...
                siemplify.LOGGER.info(
                    f"Step 3: Downloading remediation log externalId={external_id}."
                )
                file_bytes = manager.download_file(external_id)
                siemplify.LOGGER.info(
                    f"Step 3 complete: Downloaded {len(file_bytes)} bytes for '{filename}'."
                )
                ...
                output_message = (
                    f"Remediation log '{filename}' attached to case "
                ...
                raise RubrikException(f"File generation failed for type={USER_FILE_TYPE_LOG}.")
        ...
        output_message = COMMON_ACTION_ERROR_MESSAGE.format(
            DOWNLOAD_REMEDIATION_LOG_SCRIPT_NAME, e
        )
        result_value = RESULT_VALUE_FALSE
        status = EXECUTION_STATE_FAILED
        siemplify.LOGGER.error(output_message)
