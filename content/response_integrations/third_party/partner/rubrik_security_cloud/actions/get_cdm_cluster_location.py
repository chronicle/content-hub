...
from ..core.constants import (
    ...
    GET_CDM_CLUSTER_LOCATION_SCRIPT_NAME,
    MAX_TABLE_RECORDS,
    RESULT_VALUE_FALSE,
    RESULT_VALUE_TRUE,
)
from ..core.datamodels import CDMClusterLocationDatamodel
...
@output_handler
def main():
    """Retrieve CDM Cluster Location information from Rubrik Security Cloud.

    This action retrieves the geographical location information for a specific CDM
    (Cloud Data Management) cluster, including location details for all nodes in the cluster.
    ...
    siemplify.script_name = GET_CDM_CLUSTER_LOCATION_SCRIPT_NAME
    ...
    try:
        ...
        siemplify.LOGGER.info(f"Retrieving CDM Cluster Location for Cluster ID: {cluster_id}")
        response = rubrik_manager.get_cdm_cluster_location(cluster_id=cluster_id)
        ...
        location_data = CDMClusterLocationDatamodel(cluster_id, nodes)

        output_message = f"Successfully retrieved CDM Cluster Location for Cluster ID: {cluster_id}"
        table_data = location_data.to_csv()
        consise_table_data = table_data[:MAX_TABLE_RECORDS]
        if consise_table_data:
            siemplify.result.add_data_table(
                "CDM Cluster Location", construct_csv(consise_table_data), "RSC"
            )
        ...
        output_message = COMMON_ACTION_ERROR_MESSAGE.format(
            GET_CDM_CLUSTER_LOCATION_SCRIPT_NAME, str(e)
        )
        result_value = RESULT_VALUE_FALSE
        status = EXECUTION_STATE_FAILED
        siemplify.LOGGER.error(output_message)
