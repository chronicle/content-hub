...
from rubrik_security_cloud.actions import get_cdm_cluster_connection_state
from rubrik_security_cloud.tests.common import CONFIG_PATH, MOCK_CDM_CLUSTER_CONNECTION_STATE
...
class TestGetCDMClusterConnectionState:
    @set_metadata(integration_config_file_path=CONFIG_PATH, parameters=DEFAULT_PARAMETERS)
    def test_get_cdm_cluster_connection_state_success(
        self,
        script_session: RubrikSession,
        action_output: MockActionOutput,
        rubrik: RubrikSecurityCloud,
    ) -> None:
        rubrik.cdm_cluster_connection_state_response = MOCK_CDM_CLUSTER_CONNECTION_STATE
        success_output_msg = (
            "Successfully retrieved CDM Cluster Connection State for Cluster ID: test-cluster-456"
        )

        get_cdm_cluster_connection_state.main()
