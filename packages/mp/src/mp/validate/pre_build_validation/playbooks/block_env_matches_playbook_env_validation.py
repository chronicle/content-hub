...
class BlockEnvMatchesPlaybookEnvValidation:
    ...
    def run(validation_path: Path) -> None:
        ...
        """
        dependent_blocks_ids: set[str] = mp.core.utils.get_playbook_dependent_blocks_ids(validation_path)
        if not dependent_blocks_ids:
            return

        playbook: PlaybookMetadata = PlaybookMetadata.from_non_built_path(validation_path)
        playbook_env: set[str] = set(playbook.environments)
        error_msg: list[str] = []
        for block_file in validation_path.parent.iterdir():
