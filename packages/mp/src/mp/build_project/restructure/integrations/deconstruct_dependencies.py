...
PACKAGE_FILE_PATTERN: str = r"^([a-zA-Z0-9_.\-]+?)-([0-9][.0-9a-zA-Z]*)"
...
class DependencyDeconstructor:
    ...
    def _resolve_dependencies(self, required_modules: set[str]) -> Dependencies:
        ...
        package_files = [p for ext in PACAKGE_SUFFIXES for p in dependencies_dir.glob(ext)]
...
def _should_add_integration_testing(name: str, version: str) -> bool:
    return name == TIP_COMMON and int(version[0]) >= MIN_RELEVANT_TIP_COMMON_VERSION
