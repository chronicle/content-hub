...
st_valid_built_metadata_dict = st.fixed_dictionaries({
    "ResultExample": st.none() | st_json_serializable.map(json.dumps) | st.just(""),
    "ResultName": st.text(),
    "ShowResult": st.booleans(),
})

st_valid_non_built_metadata_dict = st.fixed_dictionaries({
...
st_invalid_built_dict = st.fixed_dictionaries({
    "ResultExample": st_non_json_string.filter(lambda s: s != ""),
    "ResultName": st.text(),
    "ShowResult": st.booleans(),
})

st_invalid_non_built_dict = st.fixed_dictionaries({
...
class TestValidations:
    ...
    @given(valid_non_built=st_valid_non_built_metadata_dict)
    def test_valid_non_built(self, valid_non_built: NonBuiltDynamicResultsMetadata) -> None:
        DynamicResultsMetadata.from_non_built(valid_non_built)

    @given(valid_built=st_valid_built_metadata_dict)
    def test_valid_built(self, valid_built: BuiltDynamicResultsMetadata) -> None:
        DynamicResultsMetadata.from_built(valid_built)
    ...
    def test_invalid_non_built_fails(
        ...
        with pytest.raises((ValidationError, KeyError, ValueError)):
            DynamicResultsMetadata.from_non_built(invalid_non_built).to_non_built()
