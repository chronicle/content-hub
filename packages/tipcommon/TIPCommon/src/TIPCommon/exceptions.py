...
class ParameterValidationError(Exception):
    ...
    def __init__(
        ...
        if print_error and exception is not None:
            msg = '{} - {}'.format(msg, exception)

        super(ParameterValidationError, self).__init__(
            'Invalid parameter "{0}". {1}.'.format(param_name, msg) +
            (' Wrong value provided: {}'.format(value) if print_value else '')
        )
