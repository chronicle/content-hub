...
def _extract_clean_json_result(data: Any) -> Any:
    """Extracts the clean inner JSON result from JsonResult/ScriptResultEntityData envelopes."""
    if not isinstance(data, (dict, list)):
        return data

    if isinstance(data, dict):
        # Check for jsonResult / json_none case-insensitively
        for wrapper_key in ("jsonresult", "json_none"):
            wrapper = _get_case_insensitive(data, wrapper_key)
            if isinstance(wrapper, dict):
                raw = _get_case_insensitive(wrapper, "rawjson") or _get_case_insensitive(wrapper, "content")
                if raw is not None:
                    if isinstance(raw, str):
                        with contextlib.suppress(Exception):
                            return _extract_clean_json_result(json.loads(raw))
                    return _extract_clean_json_result(raw)

        # Check for scriptResultEntityData case-insensitively
        entity_data = _get_case_insensitive(data, "scriptresultentitydata")
        if isinstance(entity_data, list) and entity_data:
            for item in entity_data:
                if isinstance(item, dict):
                    raw = _get_case_insensitive(item, "rawjson") or _get_case_insensitive(item, "content")
                    if raw is not None:
                        if isinstance(raw, str):
                            with contextlib.suppress(Exception):
                                return _extract_clean_json_result(json.loads(raw))
...
def _get_result_json_data(
        ...
        if isinstance(jr, dict):
            ...
            if isinstance(raw_json_val, str):
                ...
                if match and chronicle_soar:
                    ...
                    logger.info(f"Inflating JsonResult.RawJson resource: {resource_path}")
                    inflated = _fetch_inflated_resource(resource_path, chronicle_soar, logger)
    ...
    clean_data = _extract_clean_json_result(raw_obj)
    return clean_data
